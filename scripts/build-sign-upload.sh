...
# Requires: source ~/.secrets/vmuxterm.env && export SPARKLE_PRIVATE_KEY
...
ENTITLEMENTS="vmux.entitlements"
APP_PATH="build/Build/Products/Release/vmux.app"

# --- Pre-flight ---
source ~/.secrets/vmuxterm.env
...
if [ ! -d "GhosttyKit.xcframework" ]; then
  ...
  cd ghostty && zig build -Demit-xcframework=true -Demit-macos-app=false -Dxcframework-target=universal -Doptimize=ReleaseFast -Dsentry=false && cd ..
...
echo "Building app..."
rm -rf build/
xcodebuild -scheme vmux -configuration Release -derivedDataPath build CODE_SIGNING_ALLOWED=NO build 2>&1 | tail -5
...
/usr/libexec/PlistBuddy -c "Add :SUFeedURL string https://github.com/manaflow-ai/vmux/releases/latest/download/appcast.xml" "$APP_PLIST"
...
# --- Notarize app ---
echo "Notarizing app..."
ditto -c -k --sequesterRsrc --keepParent "$APP_PATH" vmux-notary.zip
xcrun notarytool submit vmux-notary.zip \
  --apple-id "$APPLE_ID" --team-id "$APPLE_TEAM_ID" --password "$APPLE_APP_SPECIFIC_PASSWORD" --wait
xcrun stapler staple "$APP_PATH"
xcrun stapler validate "$APP_PATH"
rm -f vmux-notary.zip
echo "App notarized"

# --- Create and notarize DMG ---
echo "Creating DMG..."
rm -f vmux-macos.dmg
create-dmg --codesign "$SIGN_HASH" vmux-macos.dmg "$APP_PATH"
echo "Notarizing DMG..."
xcrun notarytool submit vmux-macos.dmg \
  --apple-id "$APPLE_ID" --team-id "$APPLE_TEAM_ID" --password "$APPLE_APP_SPECIFIC_PASSWORD" --wait
xcrun stapler staple vmux-macos.dmg
xcrun stapler validate vmux-macos.dmg
echo "DMG notarized"

# --- Generate Sparkle appcast ---
echo "Generating appcast..."
./scripts/sparkle_generate_appcast.sh vmux-macos.dmg "$TAG" appcast.xml
...
if gh release view "$TAG" >/dev/null 2>&1; then
  ...
  for asset in vmux-macos.dmg appcast.xml; do
  ...
  if [[ "$ALLOW_OVERWRITE" == "true" ]]; then
    ...
    gh release upload "$TAG" vmux-macos.dmg appcast.xml --clobber
  else
    echo "Uploading to existing release $TAG..."
    gh release upload "$TAG" vmux-macos.dmg appcast.xml
  fi
else
  echo "Creating release $TAG and uploading..."
  gh release create "$TAG" vmux-macos.dmg appcast.xml --title "$TAG" --notes "See CHANGELOG.md for details"
fi
...
if [[ "$TAG" != *"-nightly"* ]]; then
  VERSION="${TAG#v}"
  DMG_SHA256=$(shasum -a 256 vmux-macos.dmg | cut -d' ' -f1)
  echo "Updating homebrew cask to $VERSION (SHA: $DMG_SHA256)..."
  CASK_FILE="homebrew-vmux/Casks/vmux.rb"
  if [ -f "$CASK_FILE" ]; then
...
cask "vmux" do
  ...
  url "https://github.com/manaflow-ai/vmux/releases/download/v#{version}/vmux-macos.dmg"
  name "vmux"
  desc "Lightweight native macOS terminal with vertical tabs for AI coding agents"
  homepage "https://github.com/manaflow-ai/vmux"
  ...
  app "vmux.app"
  binary "#{appdir}/vmux.app/Contents/Resources/bin/vmux"

  zap trash: [
    "~/Library/Application Support/vmux",
    "~/Library/Caches/vmux",
    "~/Library/Preferences/ai.manaflow.vmuxterm.plist",
  ]
end
CASKEOF
    cd homebrew-vmux
    ...
    echo "WARNING: homebrew-vmux submodule not found, skipping cask update"
  fi
fi

# --- Cleanup ---
rm -rf build/ vmux-macos.dmg appcast.xml
echo ""
echo "=== Release $TAG complete ==="
say "vmux release complete"
