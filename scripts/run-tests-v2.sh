...
# This runner is intended for the UTM macOS VM (ssh vmux-vm).
# It is intentionally guarded so we don't accidentally kill the host user's vmux instances.
if [ "$(id -un)" != "vmux" ]; then
  echo "ERROR: This script is intended to be run on the vmux-vm (user: vmux)." >&2
  echo "Run via: ssh vmux-vm 'cd /Users/vmux/GhosttyTabs && ./scripts/run-tests-v2.sh'" >&2
  exit 2
fi

cd "$(dirname "$0")/.."

DERIVED_DATA_PATH="$HOME/Library/Developer/Xcode/DerivedData/vmux-tests-v2"
APP="$DERIVED_DATA_PATH/Build/Products/Debug/vmux DEV.app"
RUN_TAG="tests-v2"
...
if [ ! -d "$APP" ]; then
  echo "ERROR: vmux DEV.app not found at expected path: $APP" >&2
  exit 1
fi

cleanup() {
  pkill -x "vmux DEV" || true
  pkill -x "vmux" || true
  rm -f /tmp/vmux*.sock || true
}
...
launch_and_wait() {
  ...
  for _ in {1..50}; do
    pgrep -x "vmux DEV" >/dev/null 2>&1 || break
    sleep 0.1
  done

  # Force socket mode for deterministic automation runs, independent of prior user settings.
  defaults write com.vmuxterm.app.debug socketControlMode -string full >/dev/null 2>&1 || true

  # Launch directly with UI test mode enabled so startup follows deterministic test codepaths.
  VMUX_TAG="$RUN_TAG" VMUX_UI_TEST_MODE=1 "$APP/Contents/MacOS/vmux DEV" >/dev/null 2>&1 &

  SOCK=""
  for _ in {1..120}; do
    SOCK=$(ls -t /tmp/vmux-debug*.sock /tmp/vmux*.sock 2>/dev/null | head -1 || true)
  ...
  if [ -z "$SOCK" ] || [ ! -S "$SOCK" ]; then
    echo "ERROR: Socket not ready (looked for /tmp/vmux*.sock)" >&2
    exit 1
  fi
  export VMUX_SOCKET_PATH="$SOCK"
  export VMUX_SOCKET="$SOCK"

  # Ensure LaunchServices has a visible/main window attached for rendering checks.
  VMUX_TAG="$RUN_TAG" open "$APP" >/dev/null 2>&1 || true
  sleep 0.5
...
from vmux import vmux  # type: ignore
