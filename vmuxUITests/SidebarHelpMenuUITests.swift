...
final class SidebarHelpMenuUITests: XCTestCase {
    ...
    func testHelpMenuCheckForUpdatesTriggersSidebarUpdatePill() {
        ...
        app.launchEnvironment["VMUX_UI_TEST_FEED_MODE"] = "available"
...
final class FeedbackComposerShortcutUITests: XCTestCase {
    ...
    func testCmdOptionFWorksFromSettingsWindow() {
        let app = XCUIApplication()
        app.launchEnvironment["VMUX_UI_TEST_MODE"] = "1"
        app.launchEnvironment["VMUX_UI_TEST_SHOW_SETTINGS"] = "1"
        app.launch()
        app.activate()
...
final class CommandPaletteAllSurfacesUITests: XCTestCase {
    ...
    private let hiddenSurfaceToken = "vmux-command-palette-hidden-surface"
    private let visibleSurfaceToken = "vmux-command-palette-visible-surface"

    override func setUp() {
        super.setUp()
        continueAfterFailure = false
        socketPath = "/tmp/vmux-ui-test-command-palette-\(UUID().uuidString).sock"
