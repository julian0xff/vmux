...
final class MultiWindowNotificationsUITests: XCTestCase {
    ...
    override func setUp() {
        ...
        dataPath = "/tmp/vmux-ui-test-multi-window-notifs-\(UUID().uuidString).json"
        socketPath = "/tmp/vmux-ui-test-socket-\(UUID().uuidString).sock"
    ...
    func testNotificationsRouteToCorrectWindow() {
        ...
        app.launchEnvironment["VMUX_UI_TEST_MULTI_WINDOW_NOTIF_SETUP"] = "1"
        app.launchEnvironment["VMUX_UI_TEST_MULTI_WINDOW_NOTIF_PATH"] = dataPath
        app.launchEnvironment["VMUX_TAG"] = launchTag
        app.launch()
        XCTAssertTrue(
    ...
    func testEmptyNotificationsPopoverBlocksTerminalTyping() throws {
        ...
        app.launchEnvironment["VMUX_SOCKET_PATH"] = socketPath
        app.launchEnvironment["VMUX_SOCKET_MODE"] = "allowAll"
        app.launchEnvironment["VMUX_SOCKET_ENABLE"] = "1"
        app.launchEnvironment["VMUX_UI_TEST_SOCKET_SANITY"] = "1"
        app.launchEnvironment["VMUX_TAG"] = launchTag
        app.launch()
        XCTAssertTrue(
            ensureForegroundAfterLaunch(app, timeout: 12.0),
        ...
        let marker = "vmux_notif_block_\(UUID().uuidString.replacingOccurrences(of: "-", with: "").prefix(8))"
    ...
    func testNotifyCLIDoesNotStealFocusAcrossWindows() throws {
        ...
        app.launchEnvironment["VMUX_SOCKET_ENABLE"] = "1"
        app.launchEnvironment["VMUX_UI_TEST_SOCKET_SANITY"] = "1"
        app.launchEnvironment["VMUX_UI_TEST_NOTIFY_SOURCE_TERMINAL_READY"] = "1"
        app.launchEnvironment["VMUX_UI_TEST_ENABLE_DUPLICATE_LAUNCH_OBSERVER"] = "1"
        app.launchEnvironment["VMUX_TAG"] = launchTag
        app.launch()
        XCTAssertTrue(
            ensureForegroundAfterLaunch(app, timeout: 12.0),
        ...
        let commandStatusPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("vmux-ui-test-notify-\(commandResultStem).status")
            .path
        let commandStdoutPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("vmux-ui-test-notify-\(commandResultStem).stdout")
            .path
        let commandStderrPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("vmux-ui-test-notify-\(commandResultStem).stderr")
            .path
        let commandScriptPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("vmux-ui-test-notify-\(commandResultStem).sh")
            .path
        defer {
        ...
        guard let bundledCLIPath = resolveVmuxCLIPaths(strategy: .bundledOnly).first else {
            XCTFail("Failed to locate bundled vmux CLI for notify regression test")
            return
        }
        ...
        do {
            ...
            XCTFail(
                "Failed to write delayed bundled `vmux notify` script. " +
        ...
        XCTAssertTrue(
            ...
            "Expected vmux to move to background before delayed notify command runs. state=\(app.state.rawValue)"
        )
        ...
        XCTAssertTrue(
            ...
            "Expected delayed bundled `vmux notify` command to finish without foregrounding vmux. state=\(app.state.rawValue)"
        )
        ...
        XCTAssertFalse(
            ...
            "Expected vmux to remain in background after bundled `vmux notify`. state=\(app.state.rawValue) stderr=\(notifyStderr)"
        )
        guard notifyExitStatus == "0" else {
            XCTFail(
                "Expected bundled `vmux notify` launched from the in-app shell to succeed. " +
    ...
    private func waitForVmuxPing(timeout: TimeInterval) -> (stdout: String?, stderr: String?) {
        let deadline = Date().addingTimeInterval(timeout)
        var lastStdout: String?
        var lastStderr: String?
        while Date() < deadline {
            let result = runVmuxCommand(
    ...
    private func runVmuxCommand(
        socketPath: String,
        arguments: [String],
        responseTimeoutSeconds: Double = 3.0,
        cliStrategy: VmuxCLIStrategy = .any
    ) -> (terminationStatus: Int32, stdout: String, stderr: String) {
        var args = ["--socket", socketPath]
        args.append(contentsOf: arguments)
        var environment = ProcessInfo.processInfo.environment
        environment["VMUXTERM_CLI_RESPONSE_TIMEOUT_SEC"] = String(responseTimeoutSeconds)

        let cliPaths = resolveVmuxCLIPaths(strategy: cliStrategy)
        if cliPaths.isEmpty, cliStrategy == .bundledOnly {
            return (
                terminationStatus: -1,
                stdout: "",
                stderr: "Failed to locate bundled vmux CLI"
            )
        }

        var lastPermissionFailure: (terminationStatus: Int32, stdout: String, stderr: String)?
        for cliPath in cliPaths {
            let result = executeVmuxCommand(
        ...
        if cliStrategy == .bundledOnly {
            return lastPermissionFailure ?? (
                terminationStatus: -1,
                stdout: "",
                stderr: "Bundled vmux CLI command failed without an executable path"
            )
        }

        let fallbackArgs = ["vmux"] + args
        let fallbackResult = executeVmuxCommand(
    ...
    private enum VmuxCLIStrategy: Equatable {
        case any
        case bundledOnly
    }
    ...
    private func resolveVmuxCLIPaths(strategy: VmuxCLIStrategy) -> [String] {
        ...
        if strategy == .any {
            for key in ["VMUX_UI_TEST_CLI_PATH", "VMUXTERM_CLI"] {
        ...
        candidates.append("/tmp/vmux-\(launchTag)/Build/Products/Debug/vmux DEV.app/Contents/Resources/bin/vmux")
        candidates.append("/tmp/vmux-\(launchTag)/Build/Products/Debug/vmux.app/Contents/Resources/bin/vmux")
        if strategy == .any {
            candidates.append("/tmp/vmux-\(launchTag)/Build/Products/Debug/vmux")
        }
    ...
    private func appendCLIPathCandidates(
        ...
        strategy: VmuxCLIStrategy,
        to candidates: inout [String]
    ) {
        candidates.append("\(productsDir)/vmux DEV.app/Contents/Resources/bin/vmux")
        candidates.append("\(productsDir)/vmux.app/Contents/Resources/bin/vmux")
        if strategy == .any {
            candidates.append("\(productsDir)/vmux")
        }
        ...
        for entry in entries.sorted() where entry.hasSuffix(".app") {
            let cliPath = URL(fileURLWithPath: productsDir)
                ...
                .appendingPathComponent("Contents/Resources/bin/vmux")
                .path
            candidates.append(cliPath)
        }
        if strategy == .any {
            for entry in entries.sorted() where entry == "vmux" {
                let cliPath = URL(fileURLWithPath: productsDir)
                    .appendingPathComponent(entry)
                    .path
                candidates.append(cliPath)
    ...
    private func executeVmuxCommand(
        ...
        do {
            ...
            return (
                terminationStatus: -1,
                stdout: "",
                stderr: "Failed to run vmux command: \(error.localizedDescription) (cliPath=\(executablePath))"
            )
        }
    ...
    private func resolveSocketPath(timeout: TimeInterval, requiredWorkspaceId: String? = nil) -> String? {
        ...
        while Date() < deadline {
            for candidate in primaryCandidates {
                ...
                // Primary candidate is the explicitly requested VMUX_SOCKET_PATH. If it responds,
    ...
    private func expectedSocketCandidates(includeGlobalFallback: Bool) -> [String] {
        ...
        let taggedDebugSocket = "/tmp/vmux-debug-\(launchTag).sock"
        ...
        if includeGlobalFallback {
            ...
            candidates.append("/tmp/vmux-debug.sock")
            candidates.append(stableSocketPath())
            candidates.append("/tmp/vmux.sock")
        }
    ...
    private func stableSocketPath() -> String {
        FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first?
            ...
            .appendingPathComponent("vmux.sock", isDirectory: false)
            .path ?? "/tmp/vmux.sock"
    }
    ...
    private func discoverTmpSocketCandidates(limit: Int) -> [String] {
        ...
        let matches = entries.filter { $0.hasPrefix("vmux") && $0.hasSuffix(".sock") }
