# vmux shell integration for bash

_vmux_send() {
    local payload="$1"
    if command -v ncat >/dev/null 2>&1; then
        printf '%s\n' "$payload" | ncat -w 1 -U "$VMUX_SOCKET_PATH" --send-only
    elif command -v socat >/dev/null 2>&1; then
        printf '%s\n' "$payload" | socat -T 1 - "UNIX-CONNECT:$VMUX_SOCKET_PATH"
        ...
        if printf '%s\n' "$payload" | nc -N -U "$VMUX_SOCKET_PATH" >/dev/null 2>&1; then
            :
        else
            printf '%s\n' "$payload" | nc -w 1 -U "$VMUX_SOCKET_PATH" >/dev/null 2>&1 || true
        fi
    fi
}
...
_VMUX_PWD_LAST_PWD="${_VMUX_PWD_LAST_PWD:-}"
_VMUX_GIT_LAST_PWD="${_VMUX_GIT_LAST_PWD:-}"
_VMUX_GIT_LAST_RUN="${_VMUX_GIT_LAST_RUN:-0}"
_VMUX_GIT_JOB_PID="${_VMUX_GIT_JOB_PID:-}"
_VMUX_GIT_JOB_STARTED_AT="${_VMUX_GIT_JOB_STARTED_AT:-0}"
_VMUX_GIT_HEAD_LAST_PWD="${_VMUX_GIT_HEAD_LAST_PWD:-}"
_VMUX_GIT_HEAD_PATH="${_VMUX_GIT_HEAD_PATH:-}"
_VMUX_GIT_HEAD_SIGNATURE="${_VMUX_GIT_HEAD_SIGNATURE:-}"
_VMUX_PR_POLL_PID="${_VMUX_PR_POLL_PID:-}"
_VMUX_PR_POLL_PWD="${_VMUX_PR_POLL_PWD:-}"
_VMUX_PR_POLL_INTERVAL="${_VMUX_PR_POLL_INTERVAL:-45}"
_VMUX_PR_FORCE="${_VMUX_PR_FORCE:-0}"
_VMUX_ASYNC_JOB_TIMEOUT="${_VMUX_ASYNC_JOB_TIMEOUT:-20}"

_VMUX_PORTS_LAST_RUN="${_VMUX_PORTS_LAST_RUN:-0}"
_VMUX_SHELL_ACTIVITY_LAST="${_VMUX_SHELL_ACTIVITY_LAST:-}"
_VMUX_TTY_NAME="${_VMUX_TTY_NAME:-}"
_VMUX_TTY_REPORTED="${_VMUX_TTY_REPORTED:-0}"
...
_vmux_start_pr_poll_loop() {
    ...
    _VMUX_PR_POLL_PID=$!
    disown "$_VMUX_PR_POLL_PID" 2>/dev/null || disown
}
...
_vmux_bash_preexec_hook() {
    _vmux_preexec_command
}

_vmux_prompt_command() {
    [[ -S "$VMUX_SOCKET_PATH" ]] || return 0
    [[ -n "$VMUX_TAB_ID" ]] || return 0
    [[ -n "$VMUX_PANEL_ID" ]] || return 0
    ...
    if [[ -n "$_VMUX_GIT_JOB_PID" ]] && kill -0 "$_VMUX_GIT_JOB_PID" 2>/dev/null; then
        if [[ "$pwd" != "$_VMUX_GIT_LAST_PWD" || "$git_head_changed" == "1" ]]; then
    ...
    if [[ -z "$_VMUX_GIT_JOB_PID" ]] || ! kill -0 "$_VMUX_GIT_JOB_PID" 2>/dev/null; then
    ...
    if [[ "$pwd" != "$_VMUX_PR_POLL_PWD" || "$git_head_changed" == "1" ]]; then
    ...
    if (( now - _VMUX_PORTS_LAST_RUN >= 10 )); then
        _vmux_ports_kick
    fi
}

_vmux_install_prompt_command() {
    [[ -n "${_VMUX_PROMPT_INSTALLED:-}" ]] && return 0
    _VMUX_PROMPT_INSTALLED=1
    ...
    if [[ "$decl" == "declare -a"* ]]; then
        ...
        for item in "${PROMPT_COMMAND[@]}"; do
            [[ "$item" == "_vmux_prompt_command" ]] && existing=1 && break
        done
        if (( existing == 0 )); then
            PROMPT_COMMAND=("_vmux_prompt_command" "${PROMPT_COMMAND[@]}")
        fi
    else
        case ";$PROMPT_COMMAND;" in
            ...
            *)
                if [[ -n "$PROMPT_COMMAND" ]]; then
                    ...
                    PROMPT_COMMAND="_vmux_prompt_command"
                fi
                ;;
        esac
    fi
    ...
    if (( BASH_VERSINFO[0] > 4 || (BASH_VERSINFO[0] == 4 && BASH_VERSINFO[1] >= 4) )); then
        if (( BASH_VERSINFO[0] > 5 || (BASH_VERSINFO[0] == 5 && BASH_VERSINFO[1] >= 3) )); then
            builtin readonly _VMUX_BASH_PS0='${ _vmux_bash_preexec_hook; }'
        else
            builtin readonly _VMUX_BASH_PS0='$(_vmux_bash_preexec_hook >/dev/null)'
        fi
        if [[ "$PS0" != *"${_VMUX_BASH_PS0}"* ]]; then
            PS0=$PS0"${_VMUX_BASH_PS0}"
        fi
    fi
}
