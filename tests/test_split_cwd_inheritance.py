...
Requires:
  - vmux running with allowAll socket mode
  - bash shell integration sourced (vmux-bash-integration.bash)

Run with a tagged instance:
  VMUX_TAG=<tag> python3 tests/test_split_cwd_inheritance.py
"""
...
from vmux import vmux  # noqa: E402
...
def _focus_first_surface(client: vmux) -> str:
...
def main() -> int:
    ...
    client = vmux(socket_path=socket_path)
    client.connect()

    # Use resolved paths to avoid /tmp -> /private/tmp symlink mismatch on macOS
    test_dir_a = str(Path("/tmp/vmux_split_cwd_test_a").resolve())
    test_dir_b = str(Path("/tmp/vmux_split_cwd_test_b").resolve())
