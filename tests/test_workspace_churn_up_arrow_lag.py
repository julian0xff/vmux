...
NEW_WORKSPACES = int(os.environ.get("VMUX_LAG_NEW_WORKSPACES", "20"))
SWITCH_PASSES = int(os.environ.get("VMUX_LAG_SWITCH_PASSES", "1"))
SWITCH_DELAY_S = float(os.environ.get("VMUX_LAG_SWITCH_DELAY_S", "0.06"))
HISTORY_SEED_LINES = int(os.environ.get("VMUX_LAG_HISTORY_LINES", "120"))
KEY_EVENTS = int(os.environ.get("VMUX_LAG_KEY_EVENTS", "180"))
KEY_DELAY_S = float(os.environ.get("VMUX_LAG_KEY_DELAY_S", "0.0"))
KEY_COMBO = os.environ.get("VMUX_LAG_KEY_COMBO", "up")

MAX_P95_RATIO = float(os.environ.get("VMUX_LAG_MAX_P95_RATIO", "1.70"))
MAX_AVG_RATIO = float(os.environ.get("VMUX_LAG_MAX_AVG_RATIO", "1.70"))
MAX_CHURN_P95_MS = float(os.environ.get("VMUX_LAG_MAX_CHURN_P95_MS", "35.0"))
MAX_P95_DELTA_MS = float(os.environ.get("VMUX_LAG_MAX_P95_DELTA_MS", "20.0"))
MAX_AVG_DELTA_MS = float(os.environ.get("VMUX_LAG_MAX_AVG_DELTA_MS", "12.0"))
MIN_BASELINE_P95_MS_FOR_RATIO = float(os.environ.get("VMUX_LAG_MIN_BASELINE_P95_MS_FOR_RATIO", "6.0"))
MIN_BASELINE_AVG_MS_FOR_RATIO = float(os.environ.get("VMUX_LAG_MIN_BASELINE_AVG_MS_FOR_RATIO", "4.0"))
MAX_CPU_PERCENT = float(os.environ.get("VMUX_LAG_MAX_CPU_PERCENT", "180.0"))
ENFORCE_CPU = os.environ.get("VMUX_LAG_ENFORCE_CPU", "0") == "1"
ALLOW_MAIN_SOCKET = os.environ.get("VMUX_LAG_ALLOW_MAIN_SOCKET", "0") == "1"
...
class RawSocketClient:
    ...
    def command(self, command: str, timeout_s: float = 2.0) -> str:
        if self.sock is None:
            raise vmuxError("Raw socket client not connected")
        ...
        while True:
            ...
            if remaining <= 0:
                raise vmuxError(f"Timed out waiting for response to: {command}")
            ...
            if not chunk:
                raise vmuxError("Socket closed while waiting for response")
...
def wait_for(predicate: Callable[[], bool], timeout_s: float, step_s: float = 0.05) -> None:
    ...
    while time.time() - start < timeout_s:
        if predicate():
            return
        time.sleep(step_s)
    raise vmuxError("Timed out waiting for condition")
...
def get_vmux_pid_for_socket(socket_path: Optional[str]) -> Optional[int]:
    ...
    result = subprocess.run(
        ["pgrep", "-f", r"vmux DEV.*\.app/Contents/MacOS/vmux DEV"],
        capture_output=True,
        text=True,
    )
    if result.returncode != 0:
...
def resolve_target_socket() -> str:
    socket_path = os.environ.get("VMUX_SOCKET_PATH")
    if not socket_path:
        raise vmuxError(
            "VMUX_SOCKET_PATH is required. Point it to a tagged dev socket (for example /tmp/vmux-debug-<tag>.sock)."
        )
    base = os.path.basename(socket_path)
    if not ALLOW_MAIN_SOCKET and base in {"vmux.sock", "vmux-debug.sock"}:
        raise vmuxError(
            f"Refusing to run against main socket '{socket_path}'. Set VMUX_SOCKET_PATH to a tagged dev instance."
        )
    return socket_path
...
def keep_only_first_workspace(client: vmux) -> str:
...
def create_workspaces(client: vmux, count: int) -> list[str]:
    created: list[str] = []
...
def cycle_all_workspaces(client: vmux, passes: int, delay_s: float) -> list[str]:
...
def focused_terminal_panel(client: vmux) -> str:
    surfaces = client.list_surfaces()
    if not surfaces:
        raise vmuxError("No surfaces available in selected workspace")
...
def seed_history(client: vmux, lines: int) -> None:
    for i in range(lines):
        client.send_line(f"echo vmux-lag-seed-{i}")
...
def run_baseline_scenario(client: vmux, socket_path: str) -> tuple[str, LatencyStats]:
...
def run_churn_scenario(client: vmux, socket_path: str, first_workspace_id: str) -> tuple[str, LatencyStats]:
    first_workspace_id = keep_only_first_workspace(client)
...
def main() -> int:
    ...
    client: Optional[vmux] = None
    pid: Optional[int] = None
    first_workspace_id: Optional[str] = None

    try:
        target_socket = resolve_target_socket()
        client = vmux(socket_path=target_socket)
        client.connect()
        print(f"Using socket: {client.socket_path}")

        pid = get_vmux_pid_for_socket(client.socket_path)
        if pid is None:
            print("SKIP: vmux process not found for socket")
            return 0
        ...
        if failures:
            ...
            for item in failures:
                print(f"  - {item}")
            sample_path = maybe_write_sample(pid, "vmux_workspace_churn_up_arrow_lag")
            if sample_path:
    ...
    except vmuxError as e:
        print(f"FAIL: {e}")
        sample_path = maybe_write_sample(pid, "vmux_workspace_churn_up_arrow_error")
        if sample_path:
            print(f"sample_path: {sample_path}")
        return 1
