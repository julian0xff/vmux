...
typeset -gi _vmux_redraw_done=0
typeset -g _vmux_redraw_fd=''
...
_vmux_redraw_ready() {
  emulate -L zsh
  local fd="${1:-$_vmux_redraw_fd}"
  if [[ -n "$fd" ]]; then
    zle -F "$fd"
    exec {fd}<&-
  fi
  _vmux_redraw_fd=''
  (( _vmux_redraw_done )) && return 0
  _vmux_redraw_done=1
  zle reset-prompt
}

_vmux_redraw_line_init() {
  if (( !_vmux_redraw_done )) && [[ -z "$_vmux_redraw_fd" ]]; then
    exec {_vmux_redraw_fd}< <(
      sleep 0.05
      printf 'ready\\n'
    )
    zle -F "$_vmux_redraw_fd" _vmux_redraw_ready
  fi
}

add-zsh-hook precmd _vmux_redraw_precmd
zle -N zle-line-init _vmux_redraw_line_init
""".lstrip(),
        encoding="utf-8",
    )
...
def main() -> int:
    ...
    base = Path(tempfile.mkdtemp(prefix="vmux_ghostty_prompt_redraw_"))
    try:
        home = base / "home"
