#!/usr/bin/env python3
"""
Regression test: `vmux --version` should print version text without requiring a socket.
"""
...
def main() -> int:
    ...
    if not out:
        print("FAIL: `vmux --version` produced empty stdout")
        return 1
    ...
    if out2 != out:
        print("FAIL: `vmux --version` and `vmux version` differ")
        print(f"--version: {out!r}")
        print(f"version:   {out2!r}")
        return 1

    print(f"PASS: vmux version command works ({out})")
    return 0
