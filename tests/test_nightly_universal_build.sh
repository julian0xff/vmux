...
if ! grep -Fq 'com.vmuxterm.app.nightly.universal' "$WORKFLOW_FILE"; then
  echo "FAIL: nightly workflow must set a distinct .universal bundle ID"
  exit 1
fi

if ! grep -Fq 'https://github.com/manaflow-ai/vmux/releases/download/nightly/appcast-universal.xml' "$WORKFLOW_FILE"; then
...
if ! awk '
  ...
  in_upload && /vmux-nightly-macos\*\.dmg/ { saw_arm_artifacts=1 }
  in_upload && /vmux-nightly-universal-macos\*\.dmg/ { saw_universal_artifacts=1 }
...
if ! awk '
  ...
  in_publish && /vmux-nightly-universal-macos-\$\{\{ github\.run_id \}\}\*\.dmg/ { saw_universal_immutable=1 }
  in_publish && /vmux-nightly-universal-macos\.dmg/ { saw_universal_stable=1 }
