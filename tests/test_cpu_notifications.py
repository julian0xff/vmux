...
Requires vmux to be running with socket control enabled.
"""
...
def get_vmux_pid() -> Optional[int]:
    """Get the PID of the running vmux process."""
    socket_path = os.environ.get("VMUX_SOCKET_PATH")
    if not socket_path:
        # Ask vmux.py to resolve default socket path (supports VMUX_TAG and last-socket file).
        try:
            socket_path = vmux().socket_path
        except Exception:
            socket_path = None
    ...
    result = subprocess.run(
        ["pgrep", "-f", r"vmux\.app/Contents/MacOS/vmux$"],
        capture_output=True,
        text=True,
    )
    if result.returncode != 0:
        # Try DEV build
        result = subprocess.run(
            ["pgrep", "-f", r"vmux DEV\.app/Contents/MacOS/vmux"],
            capture_output=True,
            text=True,
        )
    if result.returncode != 0:
...
def test_cpu_after_notification_burst(client: vmux, pid: int) -> tuple[bool, str]:
    """
...
def test_cpu_after_popover_close(client: vmux, pid: int) -> tuple[bool, str]:
    """
    ...
    # Ensure the correct vmux instance is frontmost (tag-safe).
    bundle_id = vmux.default_bundle_id()
    subprocess.run(
...
def test_cpu_idle_with_notifications(client: vmux, pid: int) -> tuple[bool, str]:
    """
...
def main():
    print("=" * 60)
    print("vmux Notification CPU Tests")
    print("=" * 60)
    ...
    pid = get_vmux_pid()
    if pid is None:
        print("\n❌ SKIP: vmux is not running")
        return 0

    print(f"\nFound vmux process: PID {pid}")

    # Try to connect to the socket
    client = vmux(socket_path)
    try:
        client.connect()
        print(f"Connected to {socket_path}")
    except vmuxError:
        print("\n❌ SKIP: Could not connect to vmux socket")
        print("Tip: set VMUX_TAG=<tag> or VMUX_SOCKET_PATH=<path> to target a tagged instance.")
        return 0
