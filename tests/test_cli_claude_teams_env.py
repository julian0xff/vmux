#!/usr/bin/env python3
"""
Regression test: `vmux claude-teams` injects the tmux-style auto-mode env.
"""
...
from claude_teams_test_utils import resolve_vmux_cli
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="vmux-claude-teams-env-") as td:
        tmp = Path(td)
...
printf '%s\\n' "${VMUX_CLAUDE_TEAMS_VMUX_BIN-__UNSET__}" > "$FAKE_VMUX_BIN_LOG"
printf '%s\\n' "$@" > "$FAKE_ARGV_LOG"
...
printf '%s\\n' "${VMUX_SOCKET_PATH-__UNSET__}" > "$FAKE_SOCKET_PATH_LOG"
printf '%s\\n' "${VMUX_SOCKET_PASSWORD-__UNSET__}" > "$FAKE_SOCKET_PASSWORD_LOG"
""",
        )
        ...
        env["FAKE_VMUX_BIN_LOG"] = str(vmux_bin_log)
        ...
        explicit_socket_path = str(tmp / "explicit-vmux.sock")
        ...
        if proc.returncode != 0:
            print("FAIL: `vmux claude-teams --version` exited non-zero")
        ...
        if tmux_path.startswith(str(real_bin)):
            print(f"FAIL: expected vmux tmux shim to shadow PATH, got {tmux_path!r}")
            return 1

        vmux_bin_value = read_text(vmux_bin_log)
        if not vmux_bin_value or vmux_bin_value == "__UNSET__":
            print("FAIL: missing VMUX_CLAUDE_TEAMS_VMUX_BIN")
            return 1

        if not os.path.exists(vmux_bin_value):
            print(f"FAIL: VMUX_CLAUDE_TEAMS_VMUX_BIN does not exist: {vmux_bin_value!r}")
            return 1
        ...
        if socket_path_value != explicit_socket_path:
            print(f"FAIL: expected VMUX_SOCKET_PATH={explicit_socket_path!r}, got {socket_path_value!r}")
            return 1

        socket_password_value = read_text(socket_password_log)
        if socket_password_value != explicit_socket_password:
            print(
                "FAIL: expected VMUX_SOCKET_PASSWORD to preserve the explicit CLI override, "
                f"got {socket_password_value!r}"
            )
            return 1

    print("PASS: vmux claude-teams injects the auto-mode tmux env and shim")
    return 0
