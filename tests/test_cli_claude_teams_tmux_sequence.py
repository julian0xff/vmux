#!/usr/bin/env python3
"""
Regression test: `vmux claude-teams` supports Claude's tmux teammate flow.
"""
    ...
    def handle(self, method: str, params: dict[str, object]) -> dict[str, object]:
        with self.lock:
            ...
            if method == "surface.send_text":
                return {"ok": True}
            raise RuntimeError(f"Unsupported fake vmux method: {method}")
...
class FakeVmuxUnixServer(socketserver.ThreadingUnixStreamServer):
    allow_reuse_address = True

    def __init__(self, socket_path: str, state: FakeVmuxState) -> None:
        self.state = state
        super().__init__(socket_path, FakeVmuxHandler)


class FakeVmuxHandler(socketserver.StreamRequestHandler):
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="vmux-claude-teams-seq-") as td:
        tmp = Path(td)
        home = tmp / "home"
        home.mkdir(parents=True, exist_ok=True)

        socket_path = tmp / "fake-vmux.sock"
        state = FakeVmuxState()
        server = FakeVmuxUnixServer(str(socket_path), state)
...
printf '%s\\n' "${VMUX_SOCKET_PATH-__UNSET__}" > "$FAKE_SOCKET_LOG"
...
""",
        ...
        try:
            ...
            print("FAIL: `vmux claude-teams --version` timed out")
        ...
        if socket_value != str(socket_path):
            print(f"FAIL: expected VMUX_SOCKET_PATH={socket_path}, got {socket_value!r}")
            return 1

        window_target = read_text(window_target_log)
        if window_target != "vmux:1":
            print(f"FAIL: expected tmux window target 'vmux:1', got {window_target!r}")
            return 1
    ...
    print("PASS: vmux claude-teams supports Claude's tmux teammate flow")
    return 0
