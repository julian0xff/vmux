...
vmux wrapper .zshenv should only source Ghostty zsh integration when Ghostty
...
def _run_case(
    *,
    ...
    env["VMUX_TEST_OUT"] = str(out_path)

    # Keep input deterministic and local to this test.
    for key in (
        "GHOSTTY_ZSH_ZDOTDIR",
        "VMUX_ZSH_ZDOTDIR",
        "VMUX_ORIGINAL_ZDOTDIR",
    ...
    if ghostty_enabled:
        ...
        env["VMUX_ZSH_ZDOTDIR"] = str(orig_zdotdir)
...
def main() -> int:
    ...
    base = Path("/tmp") / f"vmux_issue_734_{os.getpid()}"
    try:
