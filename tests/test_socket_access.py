...
In vmuxOnly mode (default), only processes descended from the vmux
app process can connect. External processes (e.g., SSH) are rejected.

Test strategy:
  Phase 1: vmuxOnly — external processes get rejected
  Phase 2: vmuxOnly — internal process CAN connect (inject via shell rc)
...
def _preferred_worktree_slug():
    env_slug = os.environ.get("VMUX_TAG") or os.environ.get("VMUX_BRANCH_SLUG")
    if env_slug:
...
def _derived_app_candidates_for_current_worktree():
    ...
    for info_path in info_paths:
        ...
        app_path = os.path.join(derived_root, "Build/Products/Debug/vmux DEV.app")
...
def _find_app():
    explicit = os.environ.get("VMUX_APP_PATH")
    ...
    if preferred_slug:
        preferred_tmp = []
        preferred_tmp.extend(glob.glob(f"/tmp/vmux-{preferred_slug}/Build/Products/Debug/vmux DEV*.app"))
        preferred_tmp.extend(glob.glob(f"/private/tmp/vmux-{preferred_slug}/Build/Products/Debug/vmux DEV*.app"))
    ...
    derived_candidates = glob.glob(os.path.join(
        home, "Library/Developer/Xcode/DerivedData/*/Build/Products/Debug/vmux DEV.app"
    ))
    tmp_candidates = []
    tmp_candidates.extend(glob.glob("/tmp/vmux-*/Build/Products/Debug/vmux DEV*.app"))
    tmp_candidates.extend(glob.glob("/private/tmp/vmux-*/Build/Products/Debug/vmux DEV*.app"))
...
def _find_cli(preferred_app_path: str = ""):
    ...
    if preferred_app_path:
        ...
        sibling = os.path.join(debug_dir, "vmux")
    ...
    candidates.extend(glob.glob(os.path.join(
        home, "Library/Developer/Xcode/DerivedData/*/Build/Products/Debug/vmux"
    )))
    candidates.extend(glob.glob("/tmp/vmux-*/Build/Products/Debug/vmux"))
    candidates.extend(glob.glob("/private/tmp/vmux-*/Build/Products/Debug/vmux"))
...
def _kill_vmux(app_path: str = None):
    if app_path:
        exe = os.path.join(app_path, "Contents/MacOS/vmux DEV")
        subprocess.run(["pkill", "-f", exe], capture_output=True)
    else:
        subprocess.run(["pkill", "-x", "vmux DEV"], capture_output=True)
    time.sleep(1.5)


def _launch_vmux(app_path: str, socket_path: str, mode: str = None, extra_env: dict = None):
    if os.path.exists(socket_path):
        try:
    ...
    env_args = []
    if mode:
        env_args = ["--env", f"VMUX_SOCKET_MODE={mode}"]
    launch_env = {
        "VMUX_SOCKET_PATH": socket_path,
...
def test_internal_process_allowed(socket_path: str, app_path: str) -> TestResult:
    """
    Verify a vmux-spawned terminal process CAN connect in vmuxOnly mode.
    Inject a test via the shell rc file, then launch vmux in vmuxOnly mode.
    The shell (a descendant of vmux) runs the test on startup.
    """
    result = TestResult("Internal process can connect (vmuxOnly)")
    marker = os.path.join(tempfile.gettempdir(), f"vmux_internal_{os.getpid()}")
    hook_file = os.path.join(tempfile.gettempdir(), f"vmux_rc_hook_{os.getpid()}.sh")
...
""")
        ...
        # Kill existing vmux, launch in vmuxOnly mode (default)
        _kill_vmux(app_path)
        _launch_vmux(app_path, socket_path, mode="vmuxOnly")
        ...
        if content == "OK":
            result.success("Internal process pinged socket successfully in vmuxOnly mode")
        else:
...
def test_allowall_mode_works(socket_path: str, app_path: str) -> TestResult:
    """Verify VMUX_SOCKET_MODE=allowAll bypasses ancestry check."""
    result = TestResult("allowAll mode allows external")
    try:
        _kill_vmux(app_path)
        _launch_vmux(app_path, socket_path, mode="allowAll")
...
def test_password_mode_requires_auth(socket_path: str, app_path: str) -> TestResult:
    ...
    password = f"vmux-pass-{os.getpid()}"
    try:
        _kill_vmux(app_path)
        _launch_vmux(
            app_path,
            socket_path,
            mode="password",
            extra_env={"VMUX_SOCKET_PASSWORD": password}
        )
...
def test_password_mode_cli_exit_code(socket_path: str, app_path: str) -> TestResult:
    ...
    try:
        ...
        if not cli_path:
            result.failure("Could not find vmux CLI binary")
            return result
...
def run_tests():
    print("=" * 60)
    print("vmux Socket Access Control Tests")
    print("=" * 60)
    print()

    app_path = _find_app()
    if not app_path:
        print("Error: Could not find vmux DEV.app in DerivedData")
        return 1
    print(f"App: {app_path}")

    socket_path = f"/tmp/vmux-test-socket-access-{os.getpid()}.sock"
    try:
        os.unlink(socket_path)
    except OSError:
        pass
    ...
    # ── Phase 1: vmuxOnly — external rejection ──
    print("Phase 1: vmuxOnly mode — external rejection")
    print("-" * 50)

    # Ensure vmux is running in vmuxOnly mode
    _kill_vmux(app_path)
    print("  Launching vmux in vmuxOnly mode...")
    _launch_vmux(app_path, socket_path, mode="vmuxOnly")
    ...
    # ── Phase 2: vmuxOnly — internal process CAN connect ──
    print("Phase 2: vmuxOnly mode — internal process allowed")
    print("-" * 50)
    ...
    # ── Cleanup: leave vmux in vmuxOnly mode ──
    _kill_vmux(app_path)
    _launch_vmux(app_path, socket_path, mode="vmuxOnly")
