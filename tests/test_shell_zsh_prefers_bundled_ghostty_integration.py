#!/usr/bin/env python3
"""
Regression: the vmux zsh wrapper should prefer a bundled Ghostty zsh
integration file in VMUX_SHELL_INTEGRATION_DIR over the fallback integration
under GHOSTTY_RESOURCES_DIR.

Without this, tagged vmux builds can silently load Ghostty's installed app
...
def main() -> int:
    ...
    base = Path("/tmp") / f"vmux_bundled_ghostty_zsh_{os.getpid()}"
    try:
