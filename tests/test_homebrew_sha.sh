...
# Catches issues like https://github.com/manaflow-ai/vmux/issues/110 where a race
# condition caused the cask to contain the SHA of a 404 page instead of the DMG.
set -euo pipefail

CASK_FILE="$(dirname "$0")/../homebrew-vmux/Casks/vmux.rb"

if [ ! -f "$CASK_FILE" ]; then
  echo "SKIP: homebrew-vmux submodule not initialized"
  exit 0
fi
