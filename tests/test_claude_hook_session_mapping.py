...
def run_claude_hook(
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            f"vmux claude-hook {subcommand} failed:\n"
            f"exit={proc.returncode}\nstdout={proc.stdout}\nstderr={proc.stderr}"
        )
    return proc.stdout.strip()


def wait_for_notification_count(client: vmux, minimum: int, timeout: float = 4.0) -> list[dict]:
    start = time.time()
    items: list[dict] = []
...
def main() -> int:
    ...
    state_path = Path(tempfile.gettempdir()) / f"vmux_claude_hook_state_{os.getpid()}.json"
    ...
    project_dir = Path(tempfile.gettempdir()) / f"vmux_claude_map_project_{os.getpid()}"
    ...
    try:
        with vmux() as client:
            ...
            hook_env["VMUX_SOCKET_PATH"] = client.socket_path
            hook_env["VMUX_WORKSPACE_ID"] = workspace_id
            hook_env["VMUX_SURFACE_ID"] = surface_id
            hook_env["VMUX_CLAUDE_HOOK_STATE_PATH"] = str(state_path)
    ...
    except (vmuxError, RuntimeError) as exc:
        return fail(str(exc))
    finally:
        try:
            if state_path.exists():
