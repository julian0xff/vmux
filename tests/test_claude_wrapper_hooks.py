...
def run_wrapper(*, socket_state: str, argv: list[str]) -> tuple[int, list[str], list[str], str, str]:
    with tempfile.TemporaryDirectory(prefix="vmux-claude-wrapper-test-") as td:
        tmp = Path(td)
...
""",
        ...
        make_executable(
            ...
            """#!/usr/bin/env bash
set -euo pipefail
printf '%s timeout=%s\\n' "$*" "${VMUXTERM_CLI_RESPONSE_TIMEOUT_SEC-__UNSET__}" >> "$FAKE_VMUX_LOG"
if [[ "${1:-}" == "--socket" ]]; then
  shift 2
fi
if [[ "${1:-}" == "ping" ]]; then
  if [[ "${FAKE_VMUX_PING_OK:-0}" == "1" ]]; then
    exit 0
  fi
  exit 1
fi
...
""",
        ...
        env["VMUX_SURFACE_ID"] = "surface:test"
        env["VMUX_SOCKET_PATH"] = socket_path
        ...
        env["FAKE_VMUX_PING_OK"] = "1" if socket_state == "live" else "0"
        ...
        return proc.returncode, read_lines(real_args_log), read_lines(vmux_log), proc.stderr.strip(), claudecode_value
...
def test_live_socket_injects_supported_hooks(failures: list[str]) -> None:
    code, real_argv, vmux_log, stderr, claudecode = run_wrapper(socket_state="live", argv=["hello"])
    ...
    expect(any(" ping" in line for line in vmux_log), f"live socket: expected vmux ping, got {vmux_log}", failures)
    expect(
        any("timeout=0.75" in line for line in vmux_log),
        f"live socket: expected bounded ping timeout, got {vmux_log}",
        failures,
    )
...
def test_missing_socket_skips_hook_injection(failures: list[str]) -> None:
    code, real_argv, vmux_log, stderr, claudecode = run_wrapper(socket_state="missing", argv=["hello"])
    ...
    expect(vmux_log == [], f"missing socket: expected no vmux calls, got {vmux_log}", failures)
...
def test_stale_socket_skips_hook_injection(failures: list[str]) -> None:
    code, real_argv, vmux_log, stderr, claudecode = run_wrapper(socket_state="stale", argv=["hello"])
    ...
    expect(any(" ping" in line for line in vmux_log), f"stale socket: expected vmux ping probe, got {vmux_log}", failures)
    expect(
        any("timeout=0.75" in line for line in vmux_log),
        f"stale socket: expected bounded ping timeout, got {vmux_log}",
        failures,
    )
