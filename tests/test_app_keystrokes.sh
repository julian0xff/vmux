#!/bin/bash
# Test script that sends keystrokes to vmux via AppleScript
# This tests the actual keyboard input path through the app

set -e

echo "=== vmux Keystroke Test ==="
echo ""

# Check if vmux is running
if ! pgrep -x "vmux" > /dev/null; then
    echo "Error: vmux is not running"
    echo "Please start vmux first"
    exit 1
fi

echo "vmux is running"
echo ""

# Activate vmux
osascript -e 'tell application "vmux" to activate'
sleep 0.5
...
echo "Please check the vmux window to verify:"
