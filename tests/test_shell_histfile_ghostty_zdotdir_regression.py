...
vmux also injects its own zsh integration by setting ZDOTDIR to
Resources/shell-integration. If vmux incorrectly treats Ghostty's injected
...
def main() -> int:
    ...
    vmux_wrapper_dir = root / "Resources" / "shell-integration"
    ghostty_zsh_dir = root / "ghostty" / "src" / "shell-integration" / "zsh"

    if not (vmux_wrapper_dir / ".zshenv").exists():
        print(f"SKIP: missing vmux wrapper .zshenv at {vmux_wrapper_dir}")
        return 0
    if not (ghostty_zsh_dir / ".zshenv").exists():
        print(f"SKIP: missing Ghostty zsh .zshenv at {ghostty_zsh_dir}")
        return 0

    base = Path("/tmp") / f"vmux_histfile_ghostty_stack_{os.getpid()}"
    try:
        ...
        # Simulate the buggy situation: vmux stores Ghostty's injected ZDOTDIR
        # as the "original" ZDOTDIR, then sets ZDOTDIR to its own wrapper.
        env["VMUX_ORIGINAL_ZDOTDIR"] = str(ghostty_zsh_dir)
        env["ZDOTDIR"] = str(vmux_wrapper_dir)
        env["VMUX_SHELL_INTEGRATION"] = "0"
        ...
        if seen != expected:
            ...
            print(f"  vmux_wrapper_dir={vmux_wrapper_dir}")
