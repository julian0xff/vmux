...
private final class ClaudeHookSessionStore {
    private static let defaultStatePath = "~/.vmuxterm/claude-hook-sessions.json"
    ...
    init(
        ...
        if let overridePath = processEnv["VMUX_CLAUDE_HOOK_STATE_PATH"]?.trimmingCharacters(in: .whitespacesAndNewlines),
...
private enum SocketPasswordResolver {
    private static let directoryName = "vmux"
    ...
    static func resolve(explicit: String?) -> String? {
        ...
        if let env = normalized(ProcessInfo.processInfo.environment["VMUX_SOCKET_PASSWORD"]) {
            return env
        }
        return loadFromFile()
    }
...
private enum CLISocketPathResolver {
    private static let appSupportDirectoryName = "vmux"
    private static let stableSocketFileName = "vmux.sock"
    private static let lastSocketPathFileName = "last-socket-path"
    static let legacyDefaultSocketPath = "/tmp/vmux.sock"
    private static let fallbackSocketPath = "/tmp/vmux-debug.sock"
    private static let stagingSocketPath = "/tmp/vmux-staging.sock"
    private static let legacyLastSocketPathFile = "/tmp/vmux-last-socket-path"
    ...
    private static func candidatePaths(requestedPath: String, environment: [String: String]) -> [String] {
        ...
        if let tag = normalized(environment["VMUX_TAG"]) {
            let slug = sanitizeTagSlug(tag)
            candidates.append("/tmp/vmux-debug-\(slug).sock")
            candidates.append("/tmp/vmux-\(slug).sock")
        }
    ...
    private static func discoverTaggedSockets(limit: Int) -> [String] {
        ...
        for directory in socketDiscoveryDirectories() {
            ...
            for name in entries where name.hasPrefix("vmux") && name.hasSuffix(".sock") {
...
final class SocketClient {
    ...
    private static let responseTimeoutSeconds: TimeInterval = {
        ...
        if let raw = env["VMUXTERM_CLI_RESPONSE_TIMEOUT_SEC"],
...
struct VMUXCLI {
    ...
    func run() throws {
        ...
        let envSocketPath: String? = {
            for key in ["VMUX_SOCKET_PATH", "VMUX_SOCKET"] {
        ...
        // so help text is available even when vmux is not running.
        if command != "__tmux-compat",
           ...
           (commandArgs.contains("--help") || commandArgs.contains("-h")) {
            ...
            print("Unknown command '\(command)'. Run 'vmux help' to see available commands.")
            return
        }
        ...
        case "identify":
            ...
            if includeCaller {
                ...
                let workspaceArg = idWsFlag ?? (windowId == nil ? ProcessInfo.processInfo.environment["VMUX_WORKSPACE_ID"] : nil)
                let surfaceArg = optionValue(commandArgs, name: "--surface") ?? (idWsFlag == nil && windowId == nil ? ProcessInfo.processInfo.environment["VMUX_SURFACE_ID"] : nil)
        ...
        case "new-split":
            ...
            let workspaceArg = wsArg ?? (windowId == nil ? ProcessInfo.processInfo.environment["VMUX_WORKSPACE_ID"] : nil)
            let surfaceRaw = sfArg ?? panelArg ?? (wsArg == nil && windowId == nil ? ProcessInfo.processInfo.environment["VMUX_SURFACE_ID"] : nil)
        ...
        case "close-surface":
            ...
            let workspaceArg = csWsFlag ?? (windowId == nil ? ProcessInfo.processInfo.environment["VMUX_WORKSPACE_ID"] : nil)
            let surfaceRaw = optionValue(commandArgs, name: "--surface") ?? optionValue(commandArgs, name: "--panel") ?? (csWsFlag == nil && windowId == nil ? ProcessInfo.processInfo.environment["VMUX_SURFACE_ID"] : nil)
            var params: [String: Any] = [:]
            let wsId = try normalizeWorkspaceHandle(workspaceArg, client: client)
            if let wsId { params["workspace_id"] = wsId }
            let sfId = try normalizeSurfaceHandle(surfaceRaw, client: client, workspaceHandle: wsId)
        ...
        case "trigger-flash":
            ...
            let workspaceArg = tfWsFlag ?? (windowId == nil ? ProcessInfo.processInfo.environment["VMUX_WORKSPACE_ID"] : nil)
            let surfaceArg = optionValue(commandArgs, name: "--surface") ?? optionValue(commandArgs, name: "--panel") ?? (tfWsFlag == nil && windowId == nil ? ProcessInfo.processInfo.environment["VMUX_SURFACE_ID"] : nil)
            var params: [String: Any] = [:]
            let wsId = try normalizeWorkspaceHandle(workspaceArg, client: client)
            if let wsId { params["workspace_id"] = wsId }
        ...
        case "read-screen":
            ...
            let workspaceArg = wsArg ?? (windowId == nil ? ProcessInfo.processInfo.environment["VMUX_WORKSPACE_ID"] : nil)
            let surfaceArg = sfArg ?? (wsArg == nil && windowId == nil ? ProcessInfo.processInfo.environment["VMUX_SURFACE_ID"] : nil)
        ...
        case "notify":
            ...
            let workspaceArg = notifyWsFlag ?? (windowId == nil ? ProcessInfo.processInfo.environment["VMUX_WORKSPACE_ID"] : nil)
            let surfaceArg = optionValue(commandArgs, name: "--surface") ?? (notifyWsFlag == nil && windowId == nil ? ProcessInfo.processInfo.environment["VMUX_SURFACE_ID"] : nil)
        ...
        case "clear-notifications":
            ...
            if let wsFlag = optionValue(commandArgs, name: "--workspace") {
                ...
                socketCmd += " --tab=\(wsId)"
                      ...
                      let envWs = ProcessInfo.processInfo.environment["VMUX_WORKSPACE_ID"],
        ...
        case "set-status":
            ...
            let workspaceArg = wsFlag ?? (windowId == nil ? ProcessInfo.processInfo.environment["VMUX_WORKSPACE_ID"] : nil)
            let wsId = try resolveWorkspaceId(workspaceArg, client: client)
    ...
    private func runMarkdownCommand(
        ...
        if let first = args.first, first.lowercased() == "open" {
            ...
            if let first = args.first, first.hasPrefix("-") {
                throw CLIError(
                    message:
                        "markdown open: unknown flag '\(first)'. Usage: vmux markdown open <path> [--workspace <id|ref|index>] [--surface <id|ref|index>] [--window <id|ref|index>]"
                )
            } else if let first = args.first, looksLikePath(first) || first.contains(".") {
                subArgs = args
            } else if let first = args.first {
                throw CLIError(message: "Unknown markdown subcommand: \(first). Usage: vmux markdown open <path>")
            } else {
                subArgs = []
            }
        }
        ...
        guard let rawPath = subArgs.first, !rawPath.isEmpty else {
            throw CLIError(message: "markdown open requires a file path. Usage: vmux markdown open <path>")
        }
        let trailingArgs = Array(subArgs.dropFirst())
        if let unknownFlag = trailingArgs.first(where: { $0.hasPrefix("-") }) {
            throw CLIError(
                message:
                    "markdown open: unknown flag '\(unknownFlag)'. Usage: vmux markdown open <path> [--workspace <id|ref|index>] [--surface <id|ref|index>] [--window <id|ref|index>]"
            )
        }
        if let extraArg = trailingArgs.first {
            throw CLIError(
                message:
                    "markdown open: unexpected argument '\(extraArg)'. Usage: vmux markdown open <path> [--workspace <id|ref|index>] [--surface <id|ref|index>] [--window <id|ref|index>]"
            )
        }
        ...
        let workspaceRaw = workspaceOpt ?? (windowOpt == nil ? ProcessInfo.processInfo.environment["VMUX_WORKSPACE_ID"] : nil)
        if let workspaceRaw {
    ...
    /// Open a path in vmux by creating a new workspace with the given directory.
    ...
    private func openPath(_ path: String, socketPath: String) throws {
        ...
        if (try? client.connect()) == nil {
            ...
            guard connected else {
                throw CLIError(message: "vmux app did not start in time (socket not found at \(socketPath))")
            }
    ...
    private func runFeedback(
        ...
        if emailOpt == nil && bodyOpt == nil && imagePaths.isEmpty {
            var params: [String: Any] = [:]
            let env = ProcessInfo.processInfo.environment
            if let workspaceId = env["VMUX_WORKSPACE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines),
               !workspaceId.isEmpty {
    ...
    private func runWorkspaceAction(
        ...
        let workspaceArg = workspaceOpt ?? (windowOverride == nil ? ProcessInfo.processInfo.environment["VMUX_WORKSPACE_ID"] : nil)
    ...
    private func runTabAction(
        ...
        let tabArg = tabOpt
            ...
            ?? (workspaceOpt == nil && windowOverride == nil
                ? (ProcessInfo.processInfo.environment["VMUX_TAB_ID"] ?? ProcessInfo.processInfo.environment["VMUX_SURFACE_ID"])
                : nil)
    ...
    private func runBrowserCommand(
        ...
        func displaySnapshotText(_ payload: [String: Any]) -> String {
            ...
            if url.isEmpty || url == "about:blank" {
                lines.append("hint: run 'vmux browser <surface> get url' to verify navigation")
            }
        ...
        func displayBrowserValue(_ value: Any) -> String {
            if let dict = value as? [String: Any],
               let type = dict["__vmux_t"] as? String,
               type == "undefined" {
                return "undefined"
            }
            if value is NSNull {
        ...
        if subcommand == "open" || subcommand == "open-split" || subcommand == "new" {
            ...
            let respectExternalOpenRules: Bool = {
                guard let raw = ProcessInfo.processInfo.environment["VMUX_RESPECT_EXTERNAL_OPEN_RULES"] else {
                    return false
                }
            ...
            if let outPathOpt {
                ...
                if !hasText(screenshotPath) && !hasText(screenshotURL) {
                    let outputDir = FileManager.default.temporaryDirectory
                        .appendingPathComponent("vmux-browser-screenshots-cli", isDirectory: true)
    ...
    private func subcommandUsage(_ command: String) -> String? {
        ...
        case "ping":
            ...
            Check connectivity to the vmux socket server.
            """
        case "capabilities":
            return """
            Usage: vmux capabilities
        ...
        case "welcome":
            ...
            Show a welcome screen with the vmux logo and useful shortcuts.
            Auto-runs once on first launch.
            """
        case "shortcuts":
            return """
            Usage: vmux shortcuts
        ...
        case "feedback":
            return """
            Usage: vmux feedback
                   vmux feedback --email <email> --body <text> [--image <path> ...]
        ...
        case "themes":
            ...
            Usage: vmux themes
                   vmux themes list
                   vmux themes set <theme>
                   vmux themes set --light <theme> [--dark <theme>]
                   vmux themes set --dark <theme> [--light <theme>]
                   vmux themes clear

            When run in a TTY, `vmux themes` opens an interactive theme picker with
            live app preview. Use `vmux themes list` for a plain listing.

            The picker previews the selected theme across the running vmux app and
            ...
            Commands:
              ...
              clear                     Remove the vmux theme override and fall back to other config

            Examples:
              vmux themes
              vmux themes list
              vmux themes set "Catppuccin Mocha"
              vmux themes set --light "Catppuccin Latte" --dark "Catppuccin Mocha"
              vmux themes clear
            """
        case "claude-teams":
            return String(localized: "cli.claude-teams.usage", defaultValue: """
            Usage: vmux claude-teams [claude-args...]

            Launch Claude Code with agent teams enabled.

            This command:
              - defaults Claude teammate mode to auto
              - sets a tmux-like environment so Claude auto mode uses vmux splits
            ...
            The tmux shim translates supported tmux window/pane commands into vmux
            workspace and split operations in the current vmux session.

            Examples:
              vmux claude-teams
              vmux claude-teams --continue
              vmux claude-teams --model sonnet
            """)
        case "identify":
            return """
            Usage: vmux identify [--workspace <id|ref|index>] [--surface <id|ref|index>] [--no-caller]

            Print server identity and caller context details.

            Flags:
              --workspace <id|ref|index>   Caller workspace context (default: $VMUX_WORKSPACE_ID)
              --surface <id|ref|index>     Caller surface context (default: $VMUX_SURFACE_ID)
              --no-caller                  Omit caller context from the request
            """
        case "list-windows":
            return """
            Usage: vmux list-windows

            List open windows.
            """
        case "current-window":
            return """
            Usage: vmux current-window

            Print the currently selected window ID.
            """
        case "new-window":
            return """
            Usage: vmux new-window
        ...
        case "focus-window":
            return """
            Usage: vmux focus-window --window <id|ref|index>
            ...
            Example:
              vmux focus-window --window 0
              vmux focus-window --window window:1
            """
        case "close-window":
            return """
            Usage: vmux close-window --window <id|ref|index>
            ...
            Example:
              vmux close-window --window 0
              vmux close-window --window window:1
            """
        case "move-workspace-to-window":
            return """
            Usage: vmux move-workspace-to-window --workspace <id|ref|index> --window <id|ref|index>
            ...
            Example:
              vmux move-workspace-to-window --workspace workspace:2 --window window:1
            """
        case "move-surface":
            return """
            Usage: vmux move-surface [--surface <id|ref|index> | <id|ref|index>] [flags]
            ...
            Example:
              vmux move-surface --surface surface:1 --workspace workspace:2
              vmux move-surface surface:1 --pane pane:2 --index 0
            """
        case "reorder-surface":
            return """
            Usage: vmux reorder-surface [--surface <id|ref|index> | <id|ref|index>] [flags]
            ...
            Example:
              vmux reorder-surface --surface surface:1 --index 0
              vmux reorder-surface --surface surface:3 --after surface:1
            """
        case "reorder-workspace":
            return """
            Usage: vmux reorder-workspace [--workspace <id|ref|index> | <id|ref|index>] [flags]
            ...
            Example:
              vmux reorder-workspace --workspace workspace:2 --index 0
              vmux reorder-workspace --workspace workspace:3 --after workspace:1
            """
        case "workspace-action":
            return """
            Usage: vmux workspace-action --action <name> [flags]
            ...
            Flags:
              ...
              --workspace <id|ref|index>   Target workspace (default: current/$VMUX_WORKSPACE_ID)
              --title <text>               Title for rename (or pass trailing title text)

            Example:
              vmux workspace-action --workspace workspace:2 --action pin
              vmux workspace-action --action rename --title "infra"
              vmux workspace-action close-others
            """
        case "tab-action":
            return """
            Usage: vmux tab-action --action <name> [flags]
            ...
            Flags:
              --action <name>              Action name (required if not positional)
              --tab <id|ref|index>         Target tab (accepts tab:<n> or surface:<n>; default: $VMUX_TAB_ID, then $VMUX_SURFACE_ID, then focused tab)
              --surface <id|ref|index>     Alias for --tab (backward compatibility)
              --workspace <id|ref|index>   Workspace context (default: current/$VMUX_WORKSPACE_ID)
              --title <text>               Title for rename (or pass trailing title text)
              --url <url>                  Optional URL for new-browser-right

            Example:
              vmux tab-action --tab tab:3 --action pin
              vmux tab-action --action close-right
              vmux tab-action --tab tab:2 --action rename --title "build logs"
            """
        case "rename-tab":
            return """
            Usage: vmux rename-tab [--workspace <id|ref>] [--tab <id|ref>] [--surface <id|ref>] [--] <title>

            Rename a tab (surface). Defaults to the focused tab, using:
            1) explicit --tab/--surface
            2) $VMUX_TAB_ID / $VMUX_SURFACE_ID
            3) focused tab in the resolved workspace context

            Flags:
              --workspace <id|ref>   Workspace context (default: current/$VMUX_WORKSPACE_ID)
              --tab <id|ref>         Target tab (accepts tab:<n> or surface:<n>)
              --surface <id|ref>     Alias for --tab
              --title <text>         New title (or pass trailing title)

            Example:
              vmux rename-tab "build logs"
              vmux rename-tab --tab tab:3 "staging server"
              vmux rename-tab --workspace workspace:2 --surface surface:5 --title "agent run"
            """
        case "new-workspace":
            return """
            Usage: vmux new-workspace [--cwd <path>] [--command <text>]
            ...
            Example:
              vmux new-workspace
              vmux new-workspace --cwd ~/projects/myapp
              vmux new-workspace --cwd . --command "npm test"
            """
        case "list-workspaces":
            return """
            Usage: vmux list-workspaces
        ...
        case "new-split":
            return """
            Usage: vmux new-split <left|right|up|down> [flags]

            Split the current pane in the given direction.

            Flags:
              --workspace <id|ref>   Target workspace (default: $VMUX_WORKSPACE_ID)
              --surface <id|ref>     Surface to split from (default: $VMUX_SURFACE_ID)
              --panel <id|ref>       Alias for --surface

            Example:
              vmux new-split right
              vmux new-split down --workspace workspace:1
            """
        case "list-panes":
            return """
            Usage: vmux list-panes [--workspace <id|ref>]

            List panes in a workspace.

            Flags:
              --workspace <id|ref>   Workspace context (default: $VMUX_WORKSPACE_ID)

            Example:
              vmux list-panes
              vmux list-panes --workspace workspace:2
            """
        case "list-pane-surfaces":
            return """
            Usage: vmux list-pane-surfaces [--workspace <id|ref>] [--pane <id|ref>]
            ...
            Example:
              vmux list-pane-surfaces
              vmux list-pane-surfaces --workspace workspace:2 --pane pane:1
            """
        case "tree":
            return """
            Usage: vmux tree [flags]
            ...
            Output:
              ...
              - ◀ here (caller surface where `vmux tree` was invoked)
              - workspace [selected]
              - pane [focused]
              - surface [selected]
            ...
            Example:
              vmux tree
              vmux tree --all
              vmux tree --workspace workspace:2
              vmux --json tree --all
            """
        case "focus-pane":
            return """
            Usage: vmux focus-pane [--pane <id|ref> | <id|ref>] [flags]
            ...
            Example:
              ...
              vmux focus-pane pane:1
              vmux focus-pane --pane pane:1 --workspace workspace:2
            """
        case "new-pane":
            return """
            Usage: vmux new-pane [flags]
            ...
            Example:
              vmux new-pane
              vmux new-pane --type browser --direction down --url https://example.com
            """
        case "new-surface":
            return """
            Usage: vmux new-surface [flags]
        ...
        case "close-surface":
            return """
            Usage: vmux close-surface [flags]

            Close a surface. Defaults to the focused surface if none specified.

            Flags:
              --surface <id|ref>     Surface to close (default: $VMUX_SURFACE_ID)
              --panel <id|ref>       Alias for --surface
              --workspace <id|ref>   Workspace context (default: $VMUX_WORKSPACE_ID)

            Example:
              vmux close-surface
              vmux close-surface --surface surface:3
            """
        case "drag-surface-to-split":
            return """
            Usage: vmux drag-surface-to-split --surface <id|ref> <left|right|up|down>
            ...
            Example:
              vmux drag-surface-to-split --surface surface:1 right
              vmux drag-surface-to-split --panel surface:2 down
            """
        case "refresh-surfaces":
            return """
            Usage: vmux refresh-surfaces

            Refresh surface snapshots for the focused workspace.
            """
        case "surface-health":
            return """
            Usage: vmux surface-health [--workspace <id|ref>]
        ...
        case "trigger-flash":
            return """
            Usage: vmux trigger-flash [--workspace <id|ref>] [--surface <id|ref>] [--panel <id|ref>]

            Trigger the unread flash indicator for a surface.

            Flags:
              --workspace <id|ref>   Workspace context (default: $VMUX_WORKSPACE_ID)
              --surface <id|ref>     Target surface (default: $VMUX_SURFACE_ID)
              --panel <id|ref>       Alias for --surface

            Example:
              vmux trigger-flash
              vmux trigger-flash --workspace workspace:2 --surface surface:3
            """
        case "list-panels":
            return """
            Usage: vmux list-panels [--workspace <id|ref>]
            ...
            Example:
              vmux list-panels
              vmux list-panels --workspace workspace:2
            """
        case "focus-panel":
            return """
            Usage: vmux focus-panel --panel <id|ref> [--workspace <id|ref>]
            ...
            Example:
              vmux focus-panel --panel surface:2
              vmux focus-panel --panel surface:5 --workspace workspace:2
            """
        case "close-workspace":
            return """
            Usage: vmux close-workspace --workspace <id|ref|index>
            ...
            Example:
              vmux close-workspace --workspace workspace:2
            """
        case "select-workspace":
            return """
            Usage: vmux select-workspace --workspace <id|ref|index>
            ...
            Example:
              vmux select-workspace --workspace workspace:2
              vmux select-workspace --workspace 0
            """
        case "rename-workspace", "rename-window":
            return """
            Usage: vmux rename-workspace [--workspace <id|ref|index>] [--] <title>

            Rename a workspace. Defaults to the current workspace.
            tmux-compatible alias: rename-window

            Flags:
              --workspace <id|ref|index>   Workspace to rename (default: current/$VMUX_WORKSPACE_ID)

            Example:
              vmux rename-workspace "backend logs"
              vmux rename-window --workspace workspace:2 "agent run"
            """
        case "current-workspace":
            return """
            Usage: vmux current-workspace

            Print the currently selected workspace ID.
            """
        case "capture-pane":
            return """
            Usage: vmux capture-pane [--workspace <id|ref>] [--surface <id|ref>] [--scrollback] [--lines <n>]

            tmux-compatible alias for reading terminal text from a pane.

            Flags:
              --workspace <id|ref>   Workspace context (default: $VMUX_WORKSPACE_ID)
              --surface <id|ref>     Surface context (default: $VMUX_SURFACE_ID)
              --scrollback           Include scrollback
              --lines <n>            Return only the last N lines (implies --scrollback)

            Example:
              vmux capture-pane --workspace workspace:2 --surface surface:1 --scrollback --lines 200
            """
        case "resize-pane":
            return """
            Usage: vmux resize-pane [--pane <id|ref>] [--workspace <id|ref>] [-L|-R|-U|-D] [--amount <n>]
        ...
        case "pipe-pane":
            return """
            Usage: vmux pipe-pane [--workspace <id|ref>] [--surface <id|ref>] [--command <shell-command> | <shell-command>]
        ...
        case "wait-for":
            return """
            Usage: vmux wait-for [-S|--signal] <name> [--timeout <seconds>]
        ...
        case "swap-pane":
            return """
            Usage: vmux swap-pane --pane <id|ref> --target-pane <id|ref> [--workspace <id|ref>]
        ...
        case "break-pane":
            return """
            Usage: vmux break-pane [--workspace <id|ref>] [--pane <id|ref>] [--surface <id|ref>] [--no-focus]
        ...
        case "join-pane":
            return """
            Usage: vmux join-pane --target-pane <id|ref> [--workspace <id|ref>] [--pane <id|ref>] [--surface <id|ref>] [--no-focus]
        ...
        case "last-pane":
            return """
            Usage: vmux last-pane [--workspace <id|ref>]
        ...
        case "find-window":
            return """
            Usage: vmux find-window [--content] [--select] [query]
        ...
        case "clear-history":
            return """
            Usage: vmux clear-history [--workspace <id|ref>] [--surface <id|ref>]
        ...
        case "set-hook":
            return """
            Usage: vmux set-hook [--list] [--unset <event>] | <event> <command>
        ...
        case "set-buffer":
            return """
            Usage: vmux set-buffer [--name <name>] [--] <text>
        ...
        case "paste-buffer":
            return """
            Usage: vmux paste-buffer [--name <name>] [--workspace <id|ref>] [--surface <id|ref>]
        ...
        case "list-buffers":
            return """
            Usage: vmux list-buffers

            List tmux-compat buffers.
            """
        case "respawn-pane":
            return """
            Usage: vmux respawn-pane [--workspace <id|ref>] [--surface <id|ref>] [--command <cmd> | <cmd>]
        ...
        case "display-message":
            return """
            Usage: vmux display-message [-p|--print] <text>
        ...
        case "read-screen":
            ...
            Example:
              vmux read-screen
              vmux read-screen --surface surface:2 --scrollback --lines 200
            """
        case "send":
            return """
            Usage: vmux send [flags] [--] <text>
            ...
            Example:
              vmux send "echo hello"
              vmux send --surface surface:2 "ls -la\\n"
            """
        case "send-key":
            return """
            Usage: vmux send-key [flags] [--] <key>
            ...
            Example:
              vmux send-key enter
              vmux send-key --surface surface:2 ctrl+c
            """
        case "send-panel":
            return """
            Usage: vmux send-panel --panel <id|ref> [flags] [--] <text>
            ...
            Example:
              vmux send-panel --panel surface:2 "echo hello\\n"
            """
        case "send-key-panel":
            return """
            Usage: vmux send-key-panel --panel <id|ref> [flags] [--] <key>
            ...
            Example:
              vmux send-key-panel --panel surface:2 enter
              vmux send-key-panel --panel surface:2 ctrl+c
            """
        case "notify":
            return """
            Usage: vmux notify [flags]
            ...
            Example:
              vmux notify --title "Build done" --body "All tests passed"
              vmux notify --title "Error" --subtitle "test.swift" --body "Line 42: syntax error"
            """
        case "list-notifications":
            return """
            Usage: vmux list-notifications

            List queued notifications.
            """
        case "clear-notifications":
            return """
            Usage: vmux clear-notifications

            Clear all queued notifications.
            """
        case "set-status":
            return """
            Usage: vmux set-status <key> <value> [flags]
            ...
            Example:
              vmux set-status build "compiling" --icon hammer --color "#ff9500"
              vmux set-status deploy "v1.2.3" --workspace workspace:2
            """
        case "clear-status":
            return """
            Usage: vmux clear-status <key> [flags]
            ...
            Example:
              vmux clear-status build
            """
        case "list-status":
            return """
            Usage: vmux list-status [flags]
            ...
            Example:
              vmux list-status
              vmux list-status --workspace workspace:2
            """
        case "set-progress":
            return """
            Usage: vmux set-progress <0.0-1.0> [flags]
        ...
        case "clear-progress":
            return """
            Usage: vmux clear-progress [flags]
        ...
        case "sidebar-state":
            return """
            Usage: vmux sidebar-state [flags]
        ...
        case "set-app-focus":
            return """
            Usage: vmux set-app-focus <active|inactive|clear>

            Override app focus state for notification routing tests.

            Example:
              vmux set-app-focus inactive
              vmux set-app-focus clear
            """
        case "simulate-app-active":
            return """
            Usage: vmux simulate-app-active

            Trigger the app-active handler used by notification focus tests.
            """
        case "claude-hook":
            return """
            Usage: vmux claude-hook <session-start|active|stop|idle|notification|notify|prompt-submit> [flags]
            ...
            Example:
              echo '{"session_id":"abc"}' | vmux claude-hook session-start
              echo '{}' | vmux claude-hook stop
            """
        case "browser":
            return """
            Usage: vmux browser [--surface <id|ref|index> | <surface>] <subcommand> [args]
            ...
            Subcommands:
              open|open-split|new [url] [--workspace <id|ref|index>] [--window <id|ref|index>]
                open/open-split/new default to $VMUX_WORKSPACE_ID when --workspace is omitted and --window is not set
            ...
            Example:
              vmux browser open https://example.com
              vmux browser surface:1 navigate https://google.com
              vmux browser --surface surface:1 snapshot --interactive
            """
        // Legacy browser aliases — point users to `vmux browser --help`
        case "open-browser":
            return "Legacy alias for 'vmux browser open'. Run 'vmux browser --help' for details."
        case "navigate":
            return "Legacy alias for 'vmux browser navigate'. Run 'vmux browser --help' for details."
        case "browser-back":
            return "Legacy alias for 'vmux browser back'. Run 'vmux browser --help' for details."
        case "browser-forward":
            return "Legacy alias for 'vmux browser forward'. Run 'vmux browser --help' for details."
        case "browser-reload":
            return "Legacy alias for 'vmux browser reload'. Run 'vmux browser --help' for details."
        case "get-url":
            return "Legacy alias for 'vmux browser get-url'. Run 'vmux browser --help' for details."
        case "focus-webview":
            return "Legacy alias for 'vmux browser focus-webview'. Run 'vmux browser --help' for details."
        case "is-webview-focused":
            return "Legacy alias for 'vmux browser is-webview-focused'. Run 'vmux browser --help' for details."
        case "markdown":
            return """
            Usage: vmux markdown open <path> [options]
                   vmux markdown <path>       (shorthand for 'open')
            ...
            Options:
              --workspace <id|ref|index>   Target workspace (default: $VMUX_WORKSPACE_ID)
              --surface <id|ref|index>     Source surface to split from (default: focused surface)
              --window <id|ref|index>      Target window

            Examples:
              vmux markdown open plan.md
              vmux markdown ~/project/CHANGELOG.md
              vmux markdown open ./docs/design.md --workspace 0
            """
        default:
            return nil
        }
    ...
    private static let vmuxThemeOverrideBundleIdentifier = "com.vmuxterm.app"
    private static let vmuxThemesBlockStart = "# vmux themes start"
    private static let vmuxThemesBlockEnd = "# vmux themes end"
    private static let vmuxThemesReloadNotificationName = "com.vmuxterm.themes.reload-config"
    ...
    private func runInteractiveThemes() throws {
        ...
        environment["VMUX_THEME_PICKER_CONFIG"] = try vmuxThemeOverrideConfigURL().path
        environment["VMUX_THEME_PICKER_BUNDLE_ID"] = currentVmuxAppBundleIdentifier() ?? Self.vmuxThemeOverrideBundleIdentifier
        environment["VMUX_THEME_PICKER_TARGET"] = defaultThemePickerTargetMode(current: selection).rawValue
        environment["VMUX_THEME_PICKER_COLOR_SCHEME"] = defaultAppearancePrefersDarkThemes() ? "dark" : "light"
        if let light = selection.light {
            environment["VMUX_THEME_PICKER_INITIAL_LIGHT"] = light
        }
        if let dark = selection.dark {
            environment["VMUX_THEME_PICKER_INITIAL_DARK"] = dark
        }
    ...
    private func runThemes(commandArgs: [String], jsonOutput: Bool) throws {
        ...
        default:
            if subcommand.hasPrefix("-") {
                throw CLIError(message: "Unknown themes subcommand '\(subcommand)'. Run 'vmux themes --help'.")
            }
    ...
    private func printThemesList(jsonOutput: Bool) throws {
        ...
        let configPath = try vmuxThemeOverrideConfigURL().path
    ...
    private func validatedThemeName(_ rawValue: String, availableThemes: [String]) throws -> String {
        ...
        throw CLIError(message: "Unknown theme '\(trimmed)'. Run 'vmux themes' to list available themes.")
    }
    ...
    private func themeConfigSearchURLs() -> [URL] {
        let rawPaths = [
            ...
            "~/Library/Application Support/\(Self.vmuxThemeOverrideBundleIdentifier)/config",
            "~/Library/Application Support/\(Self.vmuxThemeOverrideBundleIdentifier)/config.ghostty",
        ]
    ...
    private func vmuxThemeOverrideConfigURL() throws -> URL {
        guard let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first else {
            throw CLIError(message: "Unable to resolve Application Support directory")
        }
        return appSupport
            .appendingPathComponent(Self.vmuxThemeOverrideBundleIdentifier, isDirectory: true)
            .appendingPathComponent("config.ghostty", isDirectory: false)
    }

    private func writeManagedThemeOverride(rawThemeValue: String) throws -> URL {
        let fileManager = FileManager.default
        let configURL = try vmuxThemeOverrideConfigURL()
        ...
        let strippedContents = removingManagedThemeOverride(from: existingContents)
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let block = """
        \(Self.vmuxThemesBlockStart)
        theme = \(rawThemeValue)
        \(Self.vmuxThemesBlockEnd)
        """
    ...
    private func reloadThemesIfPossible() -> ThemeReloadStatus {
        let bundleIdentifier = currentVmuxAppBundleIdentifier() ?? Self.vmuxThemeOverrideBundleIdentifier
        DistributedNotificationCenter.default().post(
            name: Notification.Name(Self.vmuxThemesReloadNotificationName),
            object: nil,
    ...
    private func currentVmuxAppBundleIdentifier() -> String? {
        if let bundleIdentifier = ProcessInfo.processInfo.environment["VMUX_BUNDLE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines),
    ...
    private func workspaceFromArgsOrEnv(_ args: [String], windowOverride: String? = nil) -> String? {
        ...
        return ProcessInfo.processInfo.environment["VMUX_WORKSPACE_ID"]
    }
    ...
    private func treeCallerContextFromEnvironment() -> [String: Any]? {
        let env = ProcessInfo.processInfo.environment
        let workspaceRaw = env["VMUX_WORKSPACE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines)
        let surfaceRaw = env["VMUX_SURFACE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines)
        var caller: [String: Any] = [:]
    ...
    private func tmuxCallerWorkspaceHandle() -> String? {
        normalizedTmuxTarget(ProcessInfo.processInfo.environment["VMUX_WORKSPACE_ID"])
    }
    ...
    private func tmuxCallerPaneHandle() -> String? {
        guard let pane = normalizedTmuxTarget(ProcessInfo.processInfo.environment["TMUX_PANE"])
            ?? normalizedTmuxTarget(ProcessInfo.processInfo.environment["VMUX_PANE_ID"]) else {
            return nil
        }
    ...
    private func tmuxCallerSurfaceHandle() -> String? {
        normalizedTmuxTarget(ProcessInfo.processInfo.environment["VMUX_SURFACE_ID"])
    }
    ...
    private func isVmuxClaudeWrapper(at path: String) -> Bool {
        guard let data = FileManager.default.contents(atPath: path) else { return false }
        let prefixData = data.prefix(512)
        guard let prefix = String(data: prefixData, encoding: .utf8) else { return false }
        return prefix.contains("vmux claude wrapper - injects hooks and session tracking")
    }
    ...
    private func resolveClaudeExecutable(searchPath: String?) -> String? {
        ...
        for entry in entries where !entry.isEmpty {
            ...
            guard !isVmuxClaudeWrapper(at: candidate) else { continue }
            return candidate
        }
        return nil
    }
    ...
    private func configureClaudeTeamsEnvironment(
        ...
        let fakeTmuxValue: String = {
            if let focusedContext {
                ...
                return "/tmp/vmux-claude-teams/\(focusedContext.workspaceId),\(windowToken),\(focusedContext.paneHandle)"
            }
            return processEnvironment["TMUX"] ?? "/tmp/vmux-claude-teams/default,0,0"
        }()
        let fakeTmuxPane = focusedContext.map { "%\($0.paneHandle)" }
            ?? processEnvironment["TMUX_PANE"]
            ?? "%1"
        let fakeTerm = processEnvironment["VMUX_CLAUDE_TEAMS_TERM"] ?? "screen-256color"

        setenv("CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS", "1", 1)
        setenv("VMUX_CLAUDE_TEAMS_VMUX_BIN", executablePath, 1)
        setenv("PATH", updatedPath, 1)
        setenv("TMUX", fakeTmuxValue, 1)
        setenv("TMUX_PANE", fakeTmuxPane, 1)
        setenv("TERM", fakeTerm, 1)
        setenv("VMUX_SOCKET_PATH", socketPath, 1)
        setenv("VMUX_SOCKET", socketPath, 1)
        if let explicitPassword,
           !explicitPassword.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            setenv("VMUX_SOCKET_PASSWORD", explicitPassword, 1)
        }
        unsetenv("TERM_PROGRAM")
        if let focusedContext {
            setenv("VMUX_WORKSPACE_ID", focusedContext.workspaceId, 1)
            if let surfaceId = focusedContext.surfaceId, !surfaceId.isEmpty {
                setenv("VMUX_SURFACE_ID", surfaceId, 1)
            }
        }
    }
    ...
    private func createClaudeTeamsShimDirectory() throws -> URL {
        ...
        let rootPath = URL(fileURLWithPath: homePath, isDirectory: true)
            .appendingPathComponent(".vmuxterm", isDirectory: true)
        ...
        #!/usr/bin/env bash
        set -euo pipefail
        exec "${VMUX_CLAUDE_TEAMS_VMUX_BIN:-vmux}" __tmux-compat "$@"
        """
    ...
    private func runClaudeTeams(
        ...
        launcherEnvironment["VMUX_SOCKET_PATH"] = socketPath
        launcherEnvironment["VMUX_SOCKET"] = socketPath
        if let explicitPassword,
           !explicitPassword.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            launcherEnvironment["VMUX_SOCKET_PASSWORD"] = explicitPassword
        }
        let shimDirectory = try createClaudeTeamsShimDirectory()
        let executablePath = resolvedExecutableURL()?.path ?? (args.first ?? "vmux")
    ...
    private func runClaudeTeamsTmuxCompat(
        ...
        case "new-session", "new":
            ...
            if parsed.hasFlag("-A") {
                throw CLIError(message: "new-session -A is not supported in vmux claude-teams mode")
            }
        ...
        case "new-window", "neww":
            ...
            if parsed.value("-t") != nil {
                throw CLIError(message: "new-window -t is not supported in vmux claude-teams mode")
            }
            var params: [String: Any] = ["focus": false]
            if let cwd = parsed.value("-c") {
                params["cwd"] = resolvePath(cwd)
    ...
    private func tmuxCompatStoreURL() -> URL {
        let root = NSString(string: "~/.vmuxterm").expandingTildeInPath
    ...
    private func tmuxWaitForSignalURL(name: String) -> URL {
        ...
        return URL(fileURLWithPath: "/tmp/vmux-wait-for-\(String(sanitized)).sig")
    }
    ...
    private func runTmuxCompatCommand(
        ...
        case "capture-pane":
            ...
            let (sfArg, rem1) = parseOption(rem0, name: "--surface")
            let (linesArg, rem2) = parseOption(rem1, name: "--lines")
            let workspaceArg = wsArg ?? (windowOverride == nil ? ProcessInfo.processInfo.environment["VMUX_WORKSPACE_ID"] : nil)
            let surfaceArg = sfArg ?? (wsArg == nil && windowOverride == nil ? ProcessInfo.processInfo.environment["VMUX_SURFACE_ID"] : nil)
        ...
        case "popup":
            throw CLIError(message: "popup is not supported yet in vmux CLI parity mode")

        case "bind-key", "unbind-key", "copy-mode":
            throw CLIError(message: "\(command) is not supported yet in vmux CLI parity mode")
        ...
        case "display-message":
            ...
            let payload = try client.sendV2(method: "notification.create", params: ["title": "vmux", "body": message])
            if jsonOutput {
                print(jsonString(payload))
            } else {
                print(message)
    ...
    private func runClaudeHook(
        ...
        let workspaceArg = hookWsFlag ?? ProcessInfo.processInfo.environment["VMUX_WORKSPACE_ID"]
        let surfaceArg = optionValue(hookArgs, name: "--surface") ?? (hookWsFlag == nil ? ProcessInfo.processInfo.environment["VMUX_SURFACE_ID"] : nil)
        ...
        case "session-start", "active":
            ...
            let claudePid: Int? = {
                guard let raw = ProcessInfo.processInfo.environment["VMUX_CLAUDE_PID"]?
                    .trimmingCharacters(in: .whitespacesAndNewlines),
                    let pid = Int(raw),
                    pid > 0 else {
                    return nil
        ...
        case "help", "--help", "-h":
            print(
                """
                vmux claude-hook <session-start|stop|session-end|notification|prompt-submit|pre-tool-use> [--workspace <id|index>] [--surface <id|index>]
                """
            )
    ...
    private func versionSummary() -> String {
        ...
        let commit = info["VMUXCommit"].flatMap { normalizedCommitHash($0) }
        let baseSummary: String
        if let version = info["CFBundleShortVersionString"], let build = info["CFBundleVersion"] {
            baseSummary = "vmux \(version) (\(build))"
        } else if let version = info["CFBundleShortVersionString"] {
            baseSummary = "vmux \(version)"
        } else if let build = info["CFBundleVersion"] {
            baseSummary = "vmux build \(build)"
        } else {
            baseSummary = "vmux version unknown"
        }
    ...
    private func resolvedVersionInfo() -> [String: String] {
        ...
        if info["VMUXCommit"] == nil,
           let commit = normalizedCommitHash(ProcessInfo.processInfo.environment["VMUX_COMMIT"]) {
            info["VMUXCommit"] = commit
        }
    ...
    private func versionInfo(from dictionary: [String: Any]?) -> [String: String]? {
        ...
        if let commit = dictionary["VMUXCommit"] as? String,
           let normalizedCommit = normalizedCommitHash(commit) {
            info["VMUXCommit"] = normalizedCommit
        }
    ...
    private func candidateInfoPlistURLs() -> [URL] {
        ...
        while true {
            ...
            // when running a standalone vmux-cli binary from build/Debug.
    ...
    private func usage() -> String {
        return """
        vmux - control vmux via Unix socket

        Usage:
          vmux <path>                Open a directory in a new workspace (launches vmux if needed)
          vmux [global-options] <command> [options]
        ...
        Socket Auth:
          --password takes precedence, then VMUX_SOCKET_PASSWORD env var, then password saved in Settings.
        ...
        Environment:
          VMUX_WORKSPACE_ID   Auto-set in vmux terminals. Used as default --workspace for
                              ALL commands (send, list-panels, new-split, notify, etc.).
          VMUX_TAB_ID         Optional alias used by `tab-action`/`rename-tab` as default --tab.
          VMUX_SURFACE_ID     Auto-set in vmux terminals. Used as default --surface.
          VMUX_SOCKET_PATH    Override the Unix socket path. Without this, the CLI defaults
                              to ~/Library/Application Support/vmux/vmux.sock and auto-discovers tagged/debug sockets.
        """
    }
}
...
struct VMUXTermMain {
    static func main() {
        ...
        let cli = VMUXCLI(args: CommandLine.arguments)
        do {
            try cli.run()
        } catch {
