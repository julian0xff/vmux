...
class UpdateViewModel: ObservableObject {
    ...
    static func userFacingErrorMessage(for error: Swift.Error) -> String {
        ...
        if nsError.domain == SUSparkleErrorDomain {
            switch nsError.code {
            case 2001:
                return String(localized: "update.error.feedDownload.message", defaultValue: "vmux couldn't download the update feed. Check your connection and try again.")
            case 1000, 1002:
    ...
    enum ReleaseNotes {
        ...
        init?(displayVersionString: String) {
            ...
            if let semver = Self.extractSemanticVersion(from: version) {
                ...
                if let url = URL(string: "https://github.com/manaflow-ai/vmux/releases/tag/\(tag)") {
                    self = .tagged(url)
                    return
                }
            }
            ...
            if let url = URL(string: "https://github.com/manaflow-ai/vmux/commit/\(newHash)") {
                self = .commit(url)
            } else {
                return nil
            }
