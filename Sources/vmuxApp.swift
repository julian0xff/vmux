...
struct vmuxApp: App {
    ...
    init() {
        ...
        if let stored = defaults.string(forKey: SocketControlSettings.appStorageKey) {
            ...
            defaults.set(legacy ? SocketControlMode.vmuxOnly.rawValue : SocketControlMode.off.rawValue,
    ...
    private static func terminateForMissingLaunchTag() -> Never {
        let message = "error: refusing to launch untagged vmux DEV; start with ./scripts/reload.sh --tag <name> (or set VMUX_TAG for test harnesses)"
        fputs("\(message)\n", stderr)
        fflush(stderr)
        NSLog("%@", message)
        Darwin.exit(64)
    ...
    var body: some Scene {
            ...
            ContentView(updateViewModel: appDelegate.updateViewModel, windowId: primaryWindowId)
                ...
                .onAppear {
                    ...
                    if ProcessInfo.processInfo.environment["VMUX_UI_TEST_MODE"] == "1" {
                        UpdateLogStore.shared.append("ui test: vmuxApp onAppear")
                    }
#endif
                    ...
                    appDelegate.configure(tabManager: tabManager, notificationStore: notificationStore, sidebarState: sidebarState)
                    applyAppearance()
                    if ProcessInfo.processInfo.environment["VMUX_UI_TEST_SHOW_SETTINGS"] == "1" {
        ...
        .commands {
            ...
            CommandGroup(after: .newItem) {
                ...
                // was the last surface in the workspace, vmux removes the workspace and closes
    ...
    private func closePanelOrWindow() {
        if let window = NSApp.keyWindow,
           window.identifier?.rawValue == "vmux.settings" {
...
private final class SettingsAboutTitlebarDebugStore: ObservableObject {
    ...
    private func ensureToolbar(on window: NSWindow, kind: SettingsAboutWindowKind) {
        ...
        let identifier = NSToolbar.Identifier("vmux.debug.titlebar.\(kind.rawValue)")
...
private final class SettingsAboutTitlebarDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("vmux.settingsAboutTitlebarDebug")
        window.center()
...
private final class DebugWindowControlsWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("vmux.debugWindowControls")
        window.center()
...
private final class AboutWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("vmux.about")
        window.center()
...
private final class AcknowledgmentsWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("vmux.licenses")
        window.center()
...
final class SettingsWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("vmux.settings")
        window.center()
...
enum SettingsNavigationRequest {
    static let notificationName = Notification.Name("vmux.settings.navigate")
...
private final class SidebarDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("vmux.sidebarDebug")
        window.center()
...
private struct AboutPanelView: View {
    ...
    private var commit: String? {
        if let value = Bundle.main.infoDictionary?["VMUXCommit"] as? String, !value.isEmpty {
            return value
        }
        let env = ProcessInfo.processInfo.environment["VMUX_COMMIT"] ?? ""
        ...
        VStack(alignment: .center) {
            ...
            VStack(alignment: .center, spacing: 32) {
                VStack(alignment: .center, spacing: 8) {
                    Text(String(localized: "about.appName", defaultValue: "vmux"))
                        .bold()
                        .font(.title)
                ...
                VStack(spacing: 2) {
                    ...
                    let commitURL = commit.flatMap { hash in
                        URL(string: "https://github.com/manaflow-ai/vmux/commit/\(hash)")
                    }
...
private final class MenuBarExtraDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("vmux.menubarDebug")
        window.center()
...
private final class BackgroundDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("vmux.backgroundDebug")
        window.center()
...
enum WelcomeSettings {
    static let shownKey = "vmuxWelcomeShown"
}
...
struct SettingsView: View {
    ...
    @AppStorage("vmuxPortBase") private var vmuxPortBase = 9100
    @AppStorage("vmuxPortRange") private var vmuxPortRange = 10
    @AppStorage(BrowserSearchSettings.searchEngineKey) private var browserSearchEngine = BrowserSearchSettings.defaultSearchEngine.rawValue
    @AppStorage(BrowserSearchSettings.searchSuggestionsEnabledKey) private var browserSearchSuggestionsEnabled = BrowserSearchSettings.defaultSearchSuggestionsEnabled
    @AppStorage(BrowserThemeSettings.modeKey) private var browserThemeMode = BrowserThemeSettings.defaultMode.rawValue
    @AppStorage(BrowserLinkOpenSettings.openTerminalLinksInVmuxBrowserKey) private var openTerminalLinksInVmuxBrowser = BrowserLinkOpenSettings.defaultOpenTerminalLinksInVmuxBrowser
    @AppStorage(BrowserLinkOpenSettings.interceptTerminalOpenCommandInVmuxBrowserKey)
    private var interceptTerminalOpenCommandInVmuxBrowser = BrowserLinkOpenSettings.initialInterceptTerminalOpenCommandInVmuxBrowserValue()
                ...
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    SettingsCard {
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: String(localized: "settings.notifications.paneFlash.subtitle", defaultValue: "Briefly flash a blue outline when vmux highlights a pane.")
                        ) {
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: "Run a shell command when a notification arrives. $VMUX_NOTIFICATION_TITLE, $VMUX_NOTIFICATION_SUBTITLE, $VMUX_NOTIFICATION_BODY are set."
                        ) {
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: openSidebarPullRequestLinksInVmuxBrowser
                            ...
                            Toggle("", isOn: $openSidebarPullRequestLinksInVmuxBrowser)
                                .labelsHidden()
                                .controlSize(.small)
                        }
                        .disabled(sidebarHideAllDetails)
                    ...
                    SettingsCard {
                        ...
                        SettingsCardNote(String(localized: "settings.automation.socketOverrides.note", defaultValue: "Overrides: VMUX_SOCKET_ENABLE, VMUX_SOCKET_MODE, and VMUX_SOCKET_PATH (set VMUX_ALLOW_SOCKET_OVERRIDE=1 for stable/nightly builds)."))
                    }
                    ...
                    SettingsCard {
                        SettingsCardRow(
                            ...
                            subtitle: claudeCodeHooksEnabled
                                ...
                                : String(localized: "settings.automation.claudeCode.subtitleOff", defaultValue: "Claude Code runs without vmux integration.")
                        ) {
                    ...
                    SettingsCard {
                        SettingsCardRow(String(localized: "settings.automation.portBase", defaultValue: "Port Base"), subtitle: String(localized: "settings.automation.portBase.subtitle", defaultValue: "Starting port for VMUX_PORT env var."), controlWidth: pickerColumnWidth) {
                            TextField("", value: $vmuxPortBase, format: .number)
                                .textFieldStyle(.roundedBorder)
                                .multilineTextAlignment(.trailing)
                        }
                        ...
                        SettingsCardRow(String(localized: "settings.automation.portRange", defaultValue: "Port Range Size"), subtitle: String(localized: "settings.automation.portRange.subtitle", defaultValue: "Number of ports per workspace."), controlWidth: pickerColumnWidth) {
                            TextField("", value: $vmuxPortRange, format: .number)
                                .textFieldStyle(.roundedBorder)
                                .multilineTextAlignment(.trailing)
                        }
                        ...
                        SettingsCardNote(String(localized: "settings.automation.port.note", defaultValue: "Each workspace gets VMUX_PORT and VMUX_PORT_END env vars with a dedicated port range. New terminals inherit these values."))
                    }
                    ...
                    SettingsCard {
                        ...
                        SettingsCardRow(
                            ...
                            Toggle("", isOn: $openTerminalLinksInVmuxBrowser)
                                .labelsHidden()
                                .controlSize(.small)
                        }
                        ...
                        SettingsCardRow(
                            ...
                            Toggle("", isOn: $interceptTerminalOpenCommandInVmuxBrowser)
                                .labelsHidden()
                                .controlSize(.small)
                        }

                        if openTerminalLinksInVmuxBrowser || interceptTerminalOpenCommandInVmuxBrowser {
                            SettingsCardDivider()

                            VStack(alignment: .leading, spacing: 6) {
                                SettingsCardRow(
                                    String(localized: "settings.browser.hostWhitelist", defaultValue: "Hosts to Open in Embedded Browser"),
                                    subtitle: String(localized: "settings.browser.hostWhitelist.subtitle", defaultValue: "Applies to terminal link clicks and intercepted `open https://...` calls. Only these hosts open in vmux. Others open in your default browser. One host or wildcard per line (for example: example.com, *.internal.example). Leave empty to open all hosts in vmux.")
                                ) {
                                    EmptyView()
                                }
    ...
    private func resetAllSettings() {
        ...
        openTerminalLinksInVmuxBrowser = BrowserLinkOpenSettings.defaultOpenTerminalLinksInVmuxBrowser
        interceptTerminalOpenCommandInVmuxBrowser = BrowserLinkOpenSettings.defaultInterceptTerminalOpenCommandInVmuxBrowser
        ...
        openSidebarPullRequestLinksInVmuxBrowser = BrowserLinkOpenSettings.defaultOpenSidebarPullRequestLinksInVmuxBrowser
...
private struct SettingsRootView: View {
    ...
    private func configureSettingsWindow(_ window: NSWindow) {
        ...
        for index in accessories.indices.reversed() {
            ...
            guard identifier.hasPrefix("vmux.") else { continue }
