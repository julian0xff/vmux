...
private enum VmuxThemeNotifications {
...
struct VmuxCLIPathInstaller {
    ...
    init(
        ...
        bundledCLIURLProvider: @escaping () -> URL? = {
            VmuxCLIPathInstaller.defaultBundledCLIURL()
        },
        expectedBundledCLIPath: String = VmuxCLIPathInstaller.defaultBundledCLIExpectedPath(),
    ...
    private static func defaultBundledCLIURL(bundle: Bundle = .main) -> URL? {
        bundle.resourceURL?.appendingPathComponent("bin/vmux", isDirectory: false)
    }

    private static func defaultBundledCLIExpectedPath(bundle: Bundle = .main) -> String {
        bundle.bundleURL
            .appendingPathComponent("Contents/Resources/bin/vmux", isDirectory: false)
            .path
    }
...
func vmuxOwningGhosttyView(for responder: NSResponder?) -> GhosttyNSView? {
    ...
    if let view = responder as? NSView,
       let ghosttyView = vmuxOwningGhosttyView(for: view) {
        return ghosttyView
    }

    if let textView = responder as? NSTextView,
       let delegateView = textView.delegate as? NSView,
       let ghosttyView = vmuxOwningGhosttyView(for: delegateView) {
        return ghosttyView
    }
...
private func vmuxOwningGhosttyView(for view: NSView) -> GhosttyNSView? {
...
final class AppDelegate: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate, NSMenuItemValidation {
    ...
    private static func detectRunningUnderXCTest(_ env: [String: String]) -> Bool {
        ...
        if env.keys.contains(where: { $0.hasPrefix("VMUX_UI_TEST_") }) { return true }
        return false
    }
    ...
    private static let persistedWindowGeometryDefaultsKey = "vmux.session.lastWindowGeometry.v1"
    ...
    private static let didInstallWindowKeyEquivalentSwizzle: Void = {
        ...
        let swizzledSelector = #selector(NSWindow.vmux_performKeyEquivalent(with:))
    ...
    private static let didInstallWindowFirstResponderSwizzle: Void = {
        ...
        let swizzledSelector = #selector(NSWindow.vmux_makeFirstResponder(_:))
        guard let originalMethod = class_getInstanceMethod(targetClass, originalSelector),
              let swizzledMethod = class_getInstanceMethod(targetClass, swizzledSelector) else {
            return
        }
    ...
    private static let didInstallWindowSendEventSwizzle: Void = {
        ...
        let swizzledSelector = #selector(NSWindow.vmux_sendEvent(_:))
        guard let originalMethod = class_getInstanceMethod(targetClass, originalSelector),
              let swizzledMethod = class_getInstanceMethod(targetClass, swizzledSelector) else {
            return
        }
    ...
    private static let didInstallApplicationSendEventSwizzle: Void = {
        ...
        let swizzledSelector = #selector(NSApplication.vmux_applicationSendEvent(_:))
        guard let originalMethod = class_getInstanceMethod(targetClass, originalSelector),
              let swizzledMethod = class_getInstanceMethod(targetClass, swizzledSelector) else {
            return
        }
    ...
    private func childExitKeyboardProbePath() -> String? {
        ...
        guard env["VMUX_UI_TEST_CHILD_EXIT_KEYBOARD_SETUP"] == "1",
              let path = env["VMUX_UI_TEST_CHILD_EXIT_KEYBOARD_PATH"],
              !path.isEmpty else {
            return nil
        }
        return path
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        DistributedNotificationCenter.default().addObserver(
            ...
            name: VmuxThemeNotifications.reloadConfig,
            object: nil,
        ...
        VmuxMainRunLoopStallMonitor.shared.installIfNeeded()
        VmuxMainThreadTurnProfiler.shared.installIfNeeded()
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) { [weak self] in
            self?.writeUITestDiagnosticsIfNeeded(stage: "after1s")
        }
#endif

        let forceDuplicateLaunchObserver = env["VMUX_UI_TEST_ENABLE_DUPLICATE_LAUNCH_OBSERVER"] == "1"
        ...
        if !isRunningUnderXCTest {
            ...
            // first-launch permission prompts and keeps vmux aligned with the update pill UI.
        }
        ...
        if env["VMUX_UI_TEST_MODE"] == "1" {
            let trigger = env["VMUX_UI_TEST_TRIGGER_UPDATE_CHECK"] ?? "<nil>"
            let feed = env["VMUX_UI_TEST_FEED_URL"] ?? "<nil>"
            UpdateLogStore.shared.append("ui test env: trigger=\(trigger) feed=\(feed)")
        }
        if env["VMUX_UI_TEST_TRIGGER_UPDATE_CHECK"] == "1" {
    ...
    private func writeUITestDiagnosticsIfNeeded(stage: String) {
        let env = ProcessInfo.processInfo.environment
        guard let path = env["VMUX_UI_TEST_DIAGNOSTICS_PATH"], !path.isEmpty else { return }
    ...
    private func currentDisplayGeometries() -> (
        ...
        let available = NSScreen.screens.map { screen in
            SessionDisplayGeometry(
                displayID: screen.vmuxDisplayID,
                frame: screen.frame,
...
#if DEBUG
        let timingStart = VmuxTypingTiming.start()
        defer {
            VmuxTypingTiming.logDuration(
    ...
    private func windowForMainWindowId(_ windowId: UUID) -> NSWindow? {
        ...
        let expectedIdentifier = "vmux.main.\(windowId.uuidString)"
    ...
    private func mainWindowId(for window: NSWindow) -> UUID? {
        ...
        guard let rawIdentifier = window.identifier?.rawValue,
              rawIdentifier.hasPrefix("vmux.main.") else { return nil }
        let idPart = String(rawIdentifier.dropFirst("vmux.main.".count))
    ...
    func isVmuxCLIInstalledInPATH() -> Bool {
        VmuxCLIPathInstaller().isInstalled()
    }

    @objc func installVmuxCLIInPath(_ sender: Any?) {
        let installer = VmuxCLIPathInstaller()
        do {
    ...
    @objc func uninstallVmuxCLIInPath(_ sender: Any?) {
        let installer = VmuxCLIPathInstaller()
        do {
    ...
    private func setupJumpUnreadUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["VMUX_UI_TEST_JUMP_UNREAD_SETUP"] == "1" else { return }
        guard let notificationStore else { return }
    ...
    func armJumpUnreadFocusRecord(tabId: UUID, surfaceId: UUID) {
        let env = ProcessInfo.processInfo.environment
        guard let path = env["VMUX_UI_TEST_JUMP_UNREAD_PATH"], !path.isEmpty else { return }
    ...
    private func setupGotoSplitUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["VMUX_UI_TEST_GOTO_SPLIT_SETUP"] == "1" else { return }
        guard tabManager != nil else { return }

        let useGhosttyConfig = env["VMUX_UI_TEST_GOTO_SPLIT_USE_GHOSTTY_CONFIG"] == "1"
    ...
    private func isGotoSplitUITestRecordingEnabled() -> Bool {
        let env = ProcessInfo.processInfo.environment
        return env["VMUX_UI_TEST_GOTO_SPLIT_SETUP"] == "1" || env["VMUX_UI_TEST_GOTO_SPLIT_RECORD_ONLY"] == "1"
    }

    private func gotoSplitUITestDataPath() -> String? {
        guard isGotoSplitUITestRecordingEnabled() else { return nil }
        let env = ProcessInfo.processInfo.environment
        guard let path = env["VMUX_UI_TEST_GOTO_SPLIT_PATH"], !path.isEmpty else { return nil }
        return path
    }
    ...
    private func focusWebViewForGotoSplitUITest(tab: Workspace, browserPanelId: UUID, attempt: Int = 0) {
        ...
        if isWebViewFocused(browserPanel),
           let (browserPaneId, terminalPaneId) = paneIdsForGotoSplitUITest(
            ...
            if ProcessInfo.processInfo.environment["VMUX_UI_TEST_GOTO_SPLIT_INPUT_SETUP"] == "1" {
    ...
    private func setupFocusedInputForGotoSplitUITest(panel: BrowserPanel, attempt: Int = 0) {
        ...
        (() => {
          try {
            const trackerInstalled = window.__vmuxAddressBarFocusTrackerInstalled === true;
            ...
            if (!trackerInstalled || readyState !== "complete") {
              ...
              return {
                ...
                trackerInstalled,
                trackedStateId:
                  window.__vmuxAddressBarFocusState &&
                  typeof window.__vmuxAddressBarFocusState.id === "string"
                    ? window.__vmuxAddressBarFocusState.id
                    : "",
                readyState
              };
            }
            ...
            let container = document.getElementById("vmux-ui-test-focus-container");
            if (!container || !container.tagName || container.tagName.toLowerCase() !== "div") {
              container = document.createElement("div");
              container.id = "vmux-ui-test-focus-container";
            ...
            const input = ensureInput("vmux-ui-test-focus-input", "vmux-ui-focus-primary");
            const secondaryInput = ensureInput("vmux-ui-test-focus-input-secondary", "vmux-ui-focus-secondary");
            ...
            let trackedFocusId = input.getAttribute("data-vmux-addressbar-focus-id");
            if (!trackedFocusId) {
              trackedFocusId = "vmux-ui-test-focus-input-tracked";
              input.setAttribute("data-vmux-addressbar-focus-id", trackedFocusId);
            }
            ...
            if (
              !window.__vmuxAddressBarFocusState ||
              typeof window.__vmuxAddressBarFocusState.id !== "string" ||
              window.__vmuxAddressBarFocusState.id !== trackedFocusId
            ) {
              window.__vmuxAddressBarFocusState = { id: trackedFocusId, selectionStart, selectionEnd };
            }
          ...
          try {
            ...
            return {
              ...
              focusTrackerInstalled:
                window.__vmuxAddressBarFocusTrackerInstalled === true ? "true" : "false"
            };
          } catch (_) {
            return {
              id: "",
    ...
    private func setupMultiWindowNotificationsUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["VMUX_UI_TEST_MULTI_WINDOW_NOTIF_SETUP"] == "1" else { return }
        guard let path = env["VMUX_UI_TEST_MULTI_WINDOW_NOTIF_PATH"], !path.isEmpty else { return }
    ...
    private func prepareMultiWindowNotificationSourceTerminalIfNeeded(
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["VMUX_UI_TEST_NOTIFY_SOURCE_TERMINAL_READY"] == "1" else { return }
    ...
    private func publishMultiWindowNotificationSocketStateIfNeeded(at path: String) {
        let env = ProcessInfo.processInfo.environment
        guard env["VMUX_UI_TEST_SOCKET_SANITY"] == "1" else { return }

        guard let config = socketListenerConfigurationIfEnabled() else {
            writeMultiWindowNotificationTestData([
                "socketExpectedPath": env["VMUX_SOCKET_PATH"] ?? "",
                "socketMode": "off",
                "socketReady": "0",
                "socketPingResponse": "",
                "socketIsRunning": "0",
    ...
    func jumpToLatestUnread() {
        guard let notificationStore else { return }
#if DEBUG
        if ProcessInfo.processInfo.environment["VMUX_UI_TEST_JUMP_UNREAD_SETUP"] == "1" {
            writeJumpUnreadTestData([
                "jumpUnreadInvoked": "1",
    ...
    static func setWindowFirstResponderGuardTesting(currentEvent: NSEvent?, hitView: NSView?) {
        vmuxFirstResponderGuardCurrentEventOverride = currentEvent
        vmuxFirstResponderGuardHitViewOverride = hitView
    }

    static func clearWindowFirstResponderGuardTesting() {
        vmuxFirstResponderGuardCurrentEventOverride = nil
        vmuxFirstResponderGuardHitViewOverride = nil
    }
#endif
    ...
    private func installShortcutMonitor() {
        ...
        shortcutMonitor = NSEvent.addLocalMonitorForEvents(matching: [.keyDown, .keyUp, .flagsChanged]) { [weak self] event in
            ...
            if event.type == .keyDown {
                ...
                VmuxTypingTiming.logEventDelay(path: "appMonitor", event: event)
                let shortcutMonitorTraceEnabled =
                    ProcessInfo.processInfo.environment["VMUX_SHORTCUT_MONITOR_TRACE"] == "1"
                    || UserDefaults.standard.bool(forKey: "vmuxShortcutMonitorTrace")
                ...
                let shortcutTimingStart = VmuxTypingTiming.start()
#endif
    ...
    private func handleQuitShortcutWarning() -> Bool {
        ...
        let alert = NSAlert()
        alert.alertStyle = .warning
        alert.messageText = String(localized: "dialog.quitVmux.title", defaultValue: "Quit vmux?")
        alert.informativeText = String(localized: "dialog.quitVmux.message", defaultValue: "This will close all windows and workspaces.")
        alert.addButton(withTitle: String(localized: "dialog.quitVmux.quit", defaultValue: "Quit"))
        alert.addButton(withTitle: String(localized: "common.cancel", defaultValue: "Cancel"))
    ...
    private func handleCustomShortcut(event: NSEvent) -> Bool {
        ...
        if browserAddressBarFocusedPanelId != nil,
           vmuxOwningGhosttyView(for: NSApp.keyWindow?.firstResponder) != nil {
#if DEBUG
        ...
        if !normalizedFlags.contains(.command),
           let ghosttyView = vmuxOwningGhosttyView(for: NSApp.keyWindow?.firstResponder),
        ...
        if matchShortcut(
            ...
            if let targetWindow = event.window ?? NSApp.keyWindow ?? NSApp.mainWindow,
               targetWindow.identifier?.rawValue == "vmux.settings" {
                targetWindow.performClose(nil)
            } else {
                let responder = event.window?.firstResponder
                    ?? NSApp.keyWindow?.firstResponder
                    ?? NSApp.mainWindow?.firstResponder
                if let ghosttyView = vmuxOwningGhosttyView(for: responder),
    ...
    func closeMainWindowContainingTabId(_ tabId: UUID) {
        ...
        let expectedIdentifier = "vmux.main.\(context.windowId.uuidString)"
...
#if DEBUG
        let isJumpUnreadUITest = ProcessInfo.processInfo.environment["VMUX_UI_TEST_JUMP_UNREAD_SETUP"] == "1"
...
final class MenuBarExtraController: NSObject, NSMenuDelegate {
    ...
    private let quitItem = NSMenuItem(title: String(localized: "menu.quitVmux", defaultValue: "Quit vmux"), action: nil, keyEquivalent: "")
        ...
        if let button = statusItem.button {
            ...
            button.toolTip = displayedUnreadCount == 0
                ...
                : displayedUnreadCount == 1
                    ? "vmux: " + String(localized: "statusMenu.tooltip.unread.one", defaultValue: "1 unread notification")
                    : "vmux: " + String(localized: "statusMenu.tooltip.unread.other", defaultValue: "\(displayedUnreadCount) unread notifications")
        }
    }
...
#if DEBUG
private var vmuxFirstResponderGuardCurrentEventOverride: NSEvent?
private var vmuxFirstResponderGuardHitViewOverride: NSView?
#endif
private var vmuxFirstResponderGuardCurrentEventContext: NSEvent?
private var vmuxFirstResponderGuardHitViewContext: NSView?
private var vmuxFirstResponderGuardContextWindowNumber: Int?
private var vmuxBrowserReturnForwardingDepth = 0
private var vmuxWindowFirstResponderBypassDepth = 0
private var vmuxFieldEditorOwningWebViewAssociationKey: UInt8 = 0

@discardableResult
func vmuxWithWindowFirstResponderBypass<T>(_ body: () -> T) -> T {
    vmuxWindowFirstResponderBypassDepth += 1
    defer {
        vmuxWindowFirstResponderBypassDepth = max(0, vmuxWindowFirstResponderBypassDepth - 1)
    }
    return body()
}

func vmuxIsWindowFirstResponderBypassActive() -> Bool {
    vmuxWindowFirstResponderBypassDepth > 0
}

private final class VmuxFieldEditorOwningWebViewBox: NSObject {
    weak var webView: VmuxWebView?

    init(webView: VmuxWebView?) {
        self.webView = webView
    }
}

private extension NSApplication {
    @objc func vmux_applicationSendEvent(_ event: NSEvent) {
#if DEBUG
        let typingTimingStart = event.type == .keyDown ? VmuxTypingTiming.start() : nil
        let phaseTotalStart = event.type == .keyDown ? ProcessInfo.processInfo.systemUptime : 0
        if event.type == .keyDown {
            VmuxTypingTiming.logEventDelay(path: "app.sendEvent", event: event)
        }
        defer {
            if event.type == .keyDown {
                let totalMs = (ProcessInfo.processInfo.systemUptime - phaseTotalStart) * 1000.0
...
#endif
        vmux_applicationSendEvent(event)
    }
}
...
private extension NSWindow {
    @objc func vmux_makeFirstResponder(_ responder: NSResponder?) -> Bool {
        if vmuxIsWindowFirstResponderBypassActive() {
#if DEBUG
            dlog(
        ...
        let currentEvent = Self.vmuxCurrentEvent(for: self)
        let responderWebView = responder.flatMap {
            Self.vmuxOwningWebView(for: $0, in: self, event: currentEvent)
        }
        ...
        if let responder,
           ...
           !webView.allowsFirstResponderAcquisitionEffective {
            let pointerInitiatedFocus = Self.vmuxShouldAllowPointerInitiatedWebViewFocus(
                window: self,
                webView: webView,
                event: currentEvent
            )
        ...
        if pointerInitiatedWebFocus, let webView = responderWebView {
            // `NSWindow.makeFirstResponder` may run before `VmuxWebView.mouseDown(with:)`.
            // Preserve pointer intent during this synchronous responder change.
            result = webView.withPointerFocusAllowance {
                vmux_makeFirstResponder(responder)
            }
        } else {
            result = vmux_makeFirstResponder(responder)
        }
        if result {
            if let fieldEditor = responder as? NSTextView, fieldEditor.isFieldEditor {
                Self.vmuxTrackFieldEditor(fieldEditor, owningWebView: responderWebView)
    ...
    @objc func vmux_sendEvent(_ event: NSEvent) {
        ...
        let typingTimingExtra: String? = {
            ...
            let responderWebView = self.firstResponder.flatMap {
                Self.vmuxOwningWebView(for: $0, in: self, event: event)
            }
            let hitWebView = Self.vmuxHitViewForEventDispatch(in: self, event: event).flatMap {
                Self.vmuxOwningWebView(for: $0)
            }
        ...
        if event.type == .keyDown {
            VmuxTypingTiming.logEventDelay(path: "window.sendEvent", event: event)
        }
#endif
...
#endif
        let previousContextEvent = vmuxFirstResponderGuardCurrentEventContext
        let previousContextHitView = vmuxFirstResponderGuardHitViewContext
        let previousContextWindowNumber = vmuxFirstResponderGuardContextWindowNumber
        vmuxFirstResponderGuardCurrentEventContext = event
        vmuxFirstResponderGuardHitViewContext = Self.vmuxHitViewForEventDispatch(in: self, event: event)
        vmuxFirstResponderGuardContextWindowNumber = self.windowNumber
#if DEBUG
        if event.type == .keyDown {
...
#endif
        defer {
            vmuxFirstResponderGuardCurrentEventContext = previousContextEvent
            vmuxFirstResponderGuardHitViewContext = previousContextHitView
            vmuxFirstResponderGuardContextWindowNumber = previousContextWindowNumber
        }
    ...
    @objc func vmux_performKeyEquivalent(with event: NSEvent) -> Bool {
#if DEBUG
        let typingTimingStart = VmuxTypingTiming.start()
        defer {
            VmuxTypingTiming.logDuration(
        ...
        let firstResponderGhosttyView = vmuxOwningGhosttyView(for: self.firstResponder)
        ...
        if let ghosttyView = firstResponderGhosttyView {
            ...
            if ghosttyView.hasMarkedText(), !event.modifierFlags.intersection(.deviceIndependentFlagsMask).contains(.command) {
                return vmux_performKeyEquivalent(with: event)
            }
        ...
        if shouldDispatchBrowserReturnViaFirstResponderKeyDown(
            ...
            if vmuxBrowserReturnForwardingDepth > 0 {
#if DEBUG
                dlog("  → browser Return/Enter reentry; using normal dispatch")
#endif
                return false
            }
            vmuxBrowserReturnForwardingDepth += 1
            defer { vmuxBrowserReturnForwardingDepth = max(0, vmuxBrowserReturnForwardingDepth - 1) }
#if DEBUG
        ...
        let result = vmux_performKeyEquivalent(with: event)
#if DEBUG
    ...
    private static func vmuxOwningWebView(for responder: NSResponder) -> VmuxWebView? {
        if let webView = responder as? VmuxWebView {
            return webView
        }

        if let view = responder as? NSView,
           let webView = vmuxOwningWebView(for: view) {
            return webView
        }
        ...
        var current = responder.nextResponder
        while let next = current {
            if let webView = next as? VmuxWebView {
                return webView
            }
            if let view = next as? NSView,
               let webView = vmuxOwningWebView(for: view) {
    ...
    private static func vmuxOwningWebView(
        for responder: NSResponder,
        in window: NSWindow,
        event: NSEvent?
    ) -> VmuxWebView? {
        if let webView = vmuxOwningWebView(for: responder) {
            return webView
        }
        ...
        if let event,
           let hitWebView = vmuxPointerHitWebView(in: window, event: event) {
            vmuxTrackFieldEditor(textView, owningWebView: hitWebView)
            return hitWebView
        }

        return vmuxTrackedOwningWebView(for: textView)
    }

    private static func vmuxOwningWebView(for view: NSView) -> VmuxWebView? {
        if let webView = view as? VmuxWebView {
            return webView
        }

        var current: NSView? = view.superview
        while let candidate = current {
            if let webView = candidate as? VmuxWebView {
                return webView
            }
            if String(describing: type(of: candidate)).contains("WindowBrowserSlotView"),
               let portalWebView = vmuxUniqueBrowserWebView(in: candidate) {
    ...
    private static func vmuxUniqueBrowserWebView(in root: NSView) -> VmuxWebView? {
        var stack: [NSView] = [root]
        var found: VmuxWebView?
        while let current = stack.popLast() {
            if let webView = current as? VmuxWebView {
                if found == nil {
                    found = webView
    ...
    private static func vmuxCurrentEvent(for window: NSWindow) -> NSEvent? {
#if DEBUG
        if let override = vmuxFirstResponderGuardCurrentEventOverride {
            return override
        }
#endif
        if vmuxFirstResponderGuardContextWindowNumber == window.windowNumber {
            return vmuxFirstResponderGuardCurrentEventContext
        }
        return NSApp.currentEvent
    }

    private static func vmuxHitViewInThemeFrame(in window: NSWindow, event: NSEvent) -> NSView? {
    ...
    private static func vmuxHitViewInContentView(in window: NSWindow, event: NSEvent) -> NSView? {
    ...
    private static func vmuxTopHitViewForEvent(in window: NSWindow, event: NSEvent) -> NSView? {
        if let hitInThemeFrame = vmuxHitViewInThemeFrame(in: window, event: event) {
            return hitInThemeFrame
        }
        return vmuxHitViewInContentView(in: window, event: event)
    }

    private static func vmuxHitViewForEventDispatch(in window: NSWindow, event: NSEvent) -> NSView? {
        ...
        return vmuxTopHitViewForEvent(in: window, event: event)
    }

    private static func vmuxHitViewForCurrentEvent(in window: NSWindow, event: NSEvent) -> NSView? {
#if DEBUG
        if let override = vmuxFirstResponderGuardHitViewOverride {
            return override
        }
#endif
        if vmuxFirstResponderGuardContextWindowNumber == window.windowNumber,
           let contextHitView = vmuxFirstResponderGuardHitViewContext {
            return contextHitView
        }
        return vmuxTopHitViewForEvent(in: window, event: event)
    }

    private static func vmuxTrackFieldEditor(_ fieldEditor: NSTextView, owningWebView webView: VmuxWebView?) {
        if let webView {
            objc_setAssociatedObject(
                fieldEditor,
                &vmuxFieldEditorOwningWebViewAssociationKey,
                VmuxFieldEditorOwningWebViewBox(webView: webView),
    ...
    private static func vmuxTrackedOwningWebView(for fieldEditor: NSTextView) -> VmuxWebView? {
        guard let box = objc_getAssociatedObject(
            fieldEditor,
            &vmuxFieldEditorOwningWebViewAssociationKey
        ) as? VmuxFieldEditorOwningWebViewBox else {
            return nil
        }
        guard let webView = box.webView else {
            vmuxTrackFieldEditor(fieldEditor, owningWebView: nil)
            return nil
        }
        return webView
    }

    private static func vmuxIsPointerDownEvent(_ event: NSEvent) -> Bool {
        switch event.type {
    ...
    private static func vmuxPointerHitWebView(in window: NSWindow, event: NSEvent) -> VmuxWebView? {
        guard vmuxIsPointerDownEvent(event) else { return nil }
        if event.windowNumber != 0, event.windowNumber != window.windowNumber {
            return nil
        }
        if let eventWindow = event.window, eventWindow !== window {
        ...
        guard let hitView = vmuxHitViewForCurrentEvent(in: window, event: event) else {
            return nil
        }
        return vmuxOwningWebView(for: hitView)
    }

    private static func vmuxShouldAllowPointerInitiatedWebViewFocus(
        window: NSWindow,
        webView: VmuxWebView,
        event: NSEvent?
    ) -> Bool {
        guard let event,
              let hitWebView = vmuxPointerHitWebView(in: window, event: event) else {
            return false
        }
