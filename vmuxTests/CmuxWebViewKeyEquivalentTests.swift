...
private var vmuxUnitTestInspectorAssociationKey: UInt8 = 0
private var vmuxUnitTestInspectorOverrideInstalled = false

private extension VmuxWebView {
    @objc func vmuxUnitTestInspector() -> NSObject? {
        objc_getAssociatedObject(self, &vmuxUnitTestInspectorAssociationKey) as? NSObject
    }
}

private extension WKWebView {
    func vmuxSetUnitTestInspector(_ inspector: NSObject?) {
        objc_setAssociatedObject(
            self,
            &vmuxUnitTestInspectorAssociationKey,
            inspector,
...
private func installVmuxUnitTestInspectorOverride() {
    guard !vmuxUnitTestInspectorOverrideInstalled else { return }

    guard let replacementMethod = class_getInstanceMethod(
        VmuxWebView.self,
        #selector(VmuxWebView.vmuxUnitTestInspector)
    ) else {
    ...
    guard added else {
        fatalError("Unable to install VmuxWebView _inspector test override")
    }

    vmuxUnitTestInspectorOverrideInstalled = true
}
...
final class VmuxWebViewKeyEquivalentTests: XCTestCase {
    ...
    func testCmdNRoutesToMainMenuWhenWebViewIsFirstResponder() {
        ...
        let webView = VmuxWebView(frame: .zero, configuration: WKWebViewConfiguration())
    ...
    func testCanBlockFirstResponderAcquisitionWhenPaneIsUnfocused() {
        ...
        let webView = VmuxWebView(frame: container.bounds, configuration: WKWebViewConfiguration())
    ...
    func testWindowFirstResponderGuardAllowsPointerInitiatedClickFocusFromPortalHostedInspectorSibling() {
        ...
        let webView = VmuxWebView(frame: slot.bounds, configuration: WKWebViewConfiguration())
        webView.autoresizingMask = [.width, .height]
    ...
    func testWindowFirstResponderBypassBlocksSwizzledMakeFirstResponder() {
        ...
        _ = window.makeFirstResponder(nil)
        vmuxWithWindowFirstResponderBypass {
            XCTAssertFalse(
    ...
    func testCmdBacktickDoesNotRouteDirectlyToMainMenuWhenWebViewIsFirstResponder() {
        ...
        XCTAssertFalse(
            spy.invoked,
            "VmuxWebView should not route Cmd+` directly to the menu when WebKit is first responder"
        )
    }
...
final class GhosttyPasteboardHelperTests: XCTestCase {
    func testHTMLOnlyPasteboardExtractsPlainText() {
        let pasteboard = NSPasteboard(name: .init("vmux-test-html-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setString("<p>Hello <strong>world</strong></p>", forType: .html)

        XCTAssertEqual(vmuxPasteboardStringContentsForTesting(pasteboard), "Hello world")
        XCTAssertNil(vmuxPasteboardImagePathForTesting(pasteboard))
    }

    func testImageHTMLClipboardFallsBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("vmux-test-image-html-\(UUID().uuidString)"))
        pasteboard.clearContents()
        ...
        XCTAssertNil(vmuxPasteboardStringContentsForTesting(pasteboard))

        let imagePath = try XCTUnwrap(vmuxPasteboardImagePathForTesting(pasteboard))
    ...
    func testImageHTMLClipboardWithVisibleTextPrefersText() throws {
        let pasteboard = NSPasteboard(name: .init("vmux-test-image-html-text-\(UUID().uuidString)"))
        pasteboard.clearContents()
        ...
        XCTAssertEqual(vmuxPasteboardStringContentsForTesting(pasteboard), "Hello")
        XCTAssertNil(vmuxPasteboardImagePathForTesting(pasteboard))
    }

    func testJPEGClipboardFallsBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("vmux-test-jpeg-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testAttachmentOnlyRTFDClipboardFallsBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("vmux-test-rtfd-attachment-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testAttachmentOnlyRTFDNonImageClipboardDoesNotFallBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("vmux-test-rtfd-non-image-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRTFDClipboardWithVisibleTextPrefersText() throws {
        let pasteboard = NSPasteboard(name: .init("vmux-test-rtfd-text-\(UUID().uuidString)"))
        pasteboard.clearContents()
...
final class AppDelegateWindowContextRoutingTests: XCTestCase {
    private func makeMainWindow(id: UUID) -> NSWindow {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("vmux.main.\(id.uuidString)")
        return window
    }
...
final class AppDelegateLaunchServicesRegistrationTests: XCTestCase {
    func testScheduleLaunchServicesRegistrationDefersRegisterWork() {
        ...
        app.scheduleLaunchServicesBundleRegistrationForTesting(
            bundleURL: URL(fileURLWithPath: "/tmp/../tmp/vmux-launch-services-test.app"),
            scheduler: { work in
                scheduledWork = work
            },
            register: { _ in
...
@MainActor
final class VmuxWebViewContextMenuTests: XCTestCase {
    ...
    func testWillOpenMenuAddsOpenLinkInDefaultBrowserAndRoutesSelectionToDefaultBrowserOpener() {
        _ = NSApplication.shared
        let webView = VmuxWebView(frame: NSRect(x: 0, y: 0, width: 800, height: 600), configuration: WKWebViewConfiguration())
        let menu = NSMenu()
...
final class BrowserDeveloperToolsVisibilityPersistenceTests: XCTestCase {
    ...
    override class func setUp() {
        super.setUp()
        installVmuxUnitTestInspectorOverride()
    }
    ...
    private func makePanelWithInspector(
        ...
        panel.webView.vmuxSetUnitTestInspector(inspector)
...
final class GhosttyResponderResolutionTests: XCTestCase {
    ...
    func testResolvesGhosttyViewFromDescendantResponder() {
        ...
        XCTAssertTrue(vmuxOwningGhosttyView(for: descendant) === ghosttyView)
    }

    func testResolvesGhosttyViewFromGhosttyResponder() {
        let ghosttyView = GhosttyNSView(frame: NSRect(x: 0, y: 0, width: 200, height: 120))
        XCTAssertTrue(vmuxOwningGhosttyView(for: ghosttyView) === ghosttyView)
    }

    func testReturnsNilForUnrelatedResponder() {
        let view = FocusProbeView(frame: NSRect(x: 0, y: 0, width: 40, height: 40))
        XCTAssertNil(vmuxOwningGhosttyView(for: view))
    }
}
...
final class CommandPaletteRenameSelectionSettingsTests: XCTestCase {
    private let suiteName = "vmux.tests.commandPaletteRenameSelection.\(UUID().uuidString)"
...
final class WorkspaceSplitWorkingDirectoryTests: XCTestCase {
    func testNewTerminalSplitFallsBackToRequestedWorkingDirectoryWhenReportedDirectoryIsStale() {
        ...
        let requestedDirectory = "/tmp/vmux-requested-split-cwd-\(UUID().uuidString)"
...
final class WorkspacePanelGitBranchTests: XCTestCase {
    ...
    func testSidebarDerivedCollectionsMatchWhenUsingPrecomputedPanelOrder() {
        ...
        workspace.updatePanelPullRequest(
            ...
            number: 101,
            label: "PR",
            url: URL(string: "https://github.com/manaflow-ai/vmux/pull/101")!,
            status: .open
        )
        workspace.updatePanelPullRequest(
            ...
            number: 18,
            label: "MR",
            url: URL(string: "https://gitlab.com/manaflow/vmux/-/merge_requests/18")!,
            status: .merged
        )
...
final class SidebarBranchOrderingTests: XCTestCase {
        ...
        let pullRequests = SidebarBranchOrdering.orderedUniquePullRequests(
            ...
            panelPullRequests: [
                first: pullRequestState(
                    number: 337,
                    label: "PR",
                    url: "https://github.com/manaflow-ai/vmux/pull/337",
                    status: .open
                ),
                second: pullRequestState(
                    number: 18,
                    label: "MR",
                    url: "https://gitlab.com/manaflow/vmux/-/merge_requests/18",
                    status: .open
                ),
                ...
                fourth: pullRequestState(
                    number: 92,
                    label: "PR",
                    url: "https://bitbucket.org/manaflow/vmux/pull-requests/92",
                    status: .closed
                )
            ],
        ...
        let pullRequests = SidebarBranchOrdering.orderedUniquePullRequests(
            ...
            panelPullRequests: [
                first: pullRequestState(
                    number: 42,
                    label: "PR",
                    url: "https://github.com/manaflow-ai/vmux/pull/42",
                    status: .open
                ),
                second: pullRequestState(
                    number: 42,
                    label: "MR",
                    url: "https://gitlab.com/manaflow/vmux/-/merge_requests/42",
                    status: .open
                )
            ],
    ...
    func testOrderedUniquePullRequestsTreatsSameNumberAndLabelDifferentUrlsAsDistinct() {
        ...
        XCTAssertEqual(
            ...
            [
                "https://github.com/manaflow-ai/vmux/pull/42",
    ...
    func testOrderedUniquePullRequestsUsesFallbackWhenNoPanelPullRequestsExist() {
        let fallback = pullRequestState(
            number: 11,
            label: "PR",
            url: "https://github.com/manaflow-ai/vmux/pull/11",
            status: .open
        )
        let pullRequests = SidebarBranchOrdering.orderedUniquePullRequests(
            orderedPanelIds: [],
...
final class FinderServicePathResolverTests: XCTestCase {
    func testOrderedUniqueDirectoriesUsesParentForFilesAndDedupes() {
        let input: [URL] = [
            URL(fileURLWithPath: "/tmp/vmux-services/project", isDirectory: true),
            URL(fileURLWithPath: "/tmp/vmux-services/project/README.md", isDirectory: false),
            URL(fileURLWithPath: "/tmp/vmux-services/../vmux-services/project", isDirectory: true),
            URL(fileURLWithPath: "/tmp/vmux-services/other", isDirectory: true),
        ]
        ...
        XCTAssertEqual(
            directories,
            [
                "/tmp/vmux-services/project",
                "/tmp/vmux-services/other",
            ]
        )
    }
    ...
    func testOrderedUniqueDirectoriesPreservesFirstSeenOrder() {
        let input: [URL] = [
            URL(fileURLWithPath: "/tmp/vmux-services/b", isDirectory: true),
            URL(fileURLWithPath: "/tmp/vmux-services/a/file.txt", isDirectory: false),
            URL(fileURLWithPath: "/tmp/vmux-services/a", isDirectory: true),
            URL(fileURLWithPath: "/tmp/vmux-services/b/file.txt", isDirectory: false),
        ]
...
final class VSCodeServeWebURLBuilderTests: XCTestCase {
    ...
    func testOpenFolderURLAppendsFolderQueryWhilePreservingToken() {
        ...
        let url = VSCodeServeWebURLBuilder.openFolderURL(
            ...
            directoryPath: "/Users/tester/Projects/vmux"
        )

        let components = URLComponents(url: url!, resolvingAgainstBaseURL: false)
        XCTAssertEqual(components?.queryItems?.first(where: { $0.name == "tkn" })?.value, "test-token")
        XCTAssertEqual(components?.queryItems?.first(where: { $0.name == "folder" })?.value, "/Users/tester/Projects/vmux")
    }
...
final class NotificationDockBadgeTests: XCTestCase {
    ...
    func testNotificationCustomStagingPreservesSourceFileWithVmuxPrefix() {
        let suiteName = "NotificationDockBadgeTests.\(UUID().uuidString)"
        guard let defaults = UserDefaults(suiteName: suiteName) else {
            XCTFail("Failed to create isolated UserDefaults suite")
            return
        ...
        let sourceURL = soundsDirectory.appendingPathComponent(
            "vmux-custom-notification-sound.source-\(UUID().uuidString).wav",
            isDirectory: false
        )
        defer {
        ...
        XCTAssertTrue(stagedName.hasPrefix("vmux-custom-notification-sound-"))
    ...
    func testNotificationCustomUnsupportedExtensionsStageAsCaf() {
        ...
        XCTAssertTrue(stagedA.hasPrefix("vmux-custom-notification-sound-"))
    ...
    func testNotificationCustomPreparationKeepsActiveSourceMetadataSidecar() {
        ...
        let sourceURL = soundsDirectory.appendingPathComponent(
            "vmux-custom-notification-sound.metadata-\(UUID().uuidString).wav",
            isDirectory: false
        )
        do {
            try Data("test".utf8).write(to: sourceURL, options: .atomic)
    ...
    func testNotificationCustomSoundReturnsNilWhenPreparationFails() {
        ...
        let invalidSourceURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("vmux-invalid-sound-\(UUID().uuidString).mp3", isDirectory: false)
        defer {
            try? FileManager.default.removeItem(at: invalidSourceURL)
            let stagedURL = URL(fileURLWithPath: NSHomeDirectory(), isDirectory: true)
                .appendingPathComponent("Library", isDirectory: true)
                .appendingPathComponent("Sounds", isDirectory: true)
                .appendingPathComponent("vmux-custom-notification-sound.caf", isDirectory: false)
    ...
    func testNotificationCustomPreparationReportsMissingFile() {
        let missingPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("vmux-missing-\(UUID().uuidString).wav", isDirectory: false)
            .path
...
final class MenuBarBuildHintFormatterTests: XCTestCase {
    func testReleaseBuildShowsNoHint() {
        XCTAssertNil(MenuBarBuildHintFormatter.menuTitle(appName: "vmux DEV menubar-extra", isDebugBuild: false))
    }

    func testDebugBuildWithTagShowsTag() {
        XCTAssertEqual(
            MenuBarBuildHintFormatter.menuTitle(appName: "vmux DEV menubar-extra", isDebugBuild: true),
    ...
    func testDebugBuildWithoutTagShowsUntagged() {
        XCTAssertEqual(
            MenuBarBuildHintFormatter.menuTitle(appName: "vmux DEV", isDebugBuild: true),
            "Build: DEV (untagged)"
        )
    }
}
...
@MainActor
final class VmuxWebViewDragRoutingTests: XCTestCase {
    func testRejectsInternalPaneDragEvenWhenFilePromiseTypesArePresent() {
        XCTAssertTrue(
            VmuxWebView.shouldRejectInternalPaneDrag([
    ...
    func testAllowsRegularExternalFileDrops() {
        XCTAssertFalse(VmuxWebView.shouldRejectInternalPaneDrag([.fileURL]))
    }
}
...
final class InternalTabDragBundleDeclarationTests: XCTestCase {
    ...
    func testAppBundleExportsInternalDragTypes() {
        ...
        XCTAssertTrue(
            exported.contains("com.vmux.sidebar-tab-reorder"),
...
final class BrowserWindowPortalLifecycleTests: XCTestCase {
    private final class TrackingPortalWebView: WKWebView {
        ...
        @objc(_enterInWindow)
        func vmuxUnitTestEnterInWindow() {
            reattachRenderingStateCount += 1
        }

        @objc(_endDeferringViewInWindowChangesSync)
        func vmuxUnitTestEndDeferringViewInWindowChangesSync() {
            reattachRenderingStateCount += 1
        }
    }
    ...
    func testPortalSlotPinPreservesSideDockedInspectorManagedWebViewFrameOnRehost() {
        ...
        let webView = VmuxWebView(frame: NSRect(x: 0, y: 0, width: 132, height: 160), configuration: WKWebViewConfiguration())
...
final class BrowserLinkOpenSettingsTests: XCTestCase {
    ...
    func testTerminalLinksDefaultToVmuxBrowser() {
        XCTAssertTrue(BrowserLinkOpenSettings.openTerminalLinksInVmuxBrowser(defaults: defaults))
    }

    func testTerminalLinksPreferenceUsesStoredValue() {
        defaults.set(false, forKey: BrowserLinkOpenSettings.openTerminalLinksInVmuxBrowserKey)
        XCTAssertFalse(BrowserLinkOpenSettings.openTerminalLinksInVmuxBrowser(defaults: defaults))

        defaults.set(true, forKey: BrowserLinkOpenSettings.openTerminalLinksInVmuxBrowserKey)
        XCTAssertTrue(BrowserLinkOpenSettings.openTerminalLinksInVmuxBrowser(defaults: defaults))
    }

    func testSidebarPullRequestLinksDefaultToVmuxBrowser() {
        XCTAssertTrue(BrowserLinkOpenSettings.openSidebarPullRequestLinksInVmuxBrowser(defaults: defaults))
    }

    func testSidebarPullRequestLinksPreferenceUsesStoredValue() {
        defaults.set(false, forKey: BrowserLinkOpenSettings.openSidebarPullRequestLinksInVmuxBrowserKey)
        XCTAssertFalse(BrowserLinkOpenSettings.openSidebarPullRequestLinksInVmuxBrowser(defaults: defaults))

        defaults.set(true, forKey: BrowserLinkOpenSettings.openSidebarPullRequestLinksInVmuxBrowserKey)
        XCTAssertTrue(BrowserLinkOpenSettings.openSidebarPullRequestLinksInVmuxBrowser(defaults: defaults))
    }

    func testOpenCommandInterceptionDefaultsToVmuxBrowser() {
        XCTAssertTrue(BrowserLinkOpenSettings.interceptTerminalOpenCommandInVmuxBrowser(defaults: defaults))
    }

    func testOpenCommandInterceptionUsesStoredValue() {
        defaults.set(false, forKey: BrowserLinkOpenSettings.interceptTerminalOpenCommandInVmuxBrowserKey)
        XCTAssertFalse(BrowserLinkOpenSettings.interceptTerminalOpenCommandInVmuxBrowser(defaults: defaults))

        defaults.set(true, forKey: BrowserLinkOpenSettings.interceptTerminalOpenCommandInVmuxBrowserKey)
        XCTAssertTrue(BrowserLinkOpenSettings.interceptTerminalOpenCommandInVmuxBrowser(defaults: defaults))
    }
    ...
    func testSettingsInitialOpenCommandInterceptionValueFallsBackToLegacyLinkToggleWhenUnset() {
        defaults.set(false, forKey: BrowserLinkOpenSettings.openTerminalLinksInVmuxBrowserKey)
        XCTAssertFalse(BrowserLinkOpenSettings.initialInterceptTerminalOpenCommandInVmuxBrowserValue(defaults: defaults))

        defaults.set(true, forKey: BrowserLinkOpenSettings.openTerminalLinksInVmuxBrowserKey)
        XCTAssertTrue(BrowserLinkOpenSettings.initialInterceptTerminalOpenCommandInVmuxBrowserValue(defaults: defaults))
    }
...
final class TerminalOpenURLTargetResolutionTests: XCTestCase {
    ...
    func testResolvesFileSchemeAsExternal() throws {
        let target = try XCTUnwrap(resolveTerminalOpenURLTarget("file:///tmp/vmux.txt"))
        switch target {
        case let .external(url):
            XCTAssertTrue(url.isFileURL)
            XCTAssertEqual(url.path, "/tmp/vmux.txt")
        default:
    ...
    func testResolvesAbsolutePathAsExternalFileURL() throws {
        let target = try XCTUnwrap(resolveTerminalOpenURLTarget("/tmp/vmux-path.txt"))
        switch target {
        case let .external(url):
            XCTAssertTrue(url.isFileURL)
            XCTAssertEqual(url.path, "/tmp/vmux-path.txt")
        default:
    ...
    func testResolvesHostlessHTTPSAsExternal() throws {
        let target = try XCTUnwrap(resolveTerminalOpenURLTarget("https:///tmp/vmux.txt"))
        switch target {
        case let .external(url):
            XCTAssertEqual(url.scheme, "https")
...
final class BrowserNavigableURLResolutionTests: XCTestCase {
    func testResolvesFileSchemeAsNavigableURL() throws {
        let resolved = try XCTUnwrap(resolveBrowserNavigableURL("file:///tmp/vmux-local-test.html"))
        XCTAssertTrue(resolved.isFileURL)
        XCTAssertEqual(resolved.path, "/tmp/vmux-local-test.html")
    }
...
final class BrowserExternalNavigationSchemeTests: XCTestCase {
    ...
    func testEmbeddedBrowserSchemesStayInWebView() throws {
        ...
        let file = try XCTUnwrap(URL(string: "file:///tmp/vmux-local-test.html"))
...
final class TerminalControllerSidebarDedupeTests: XCTestCase {
    ...
    func testNormalizeReportedDirectoryTrimsWhitespace() {
        XCTAssertEqual(
            TerminalController.normalizeReportedDirectory("   /Users/vmux/project   "),
            "/Users/vmux/project"
        )
    }
    ...
    func testNormalizeReportedDirectoryResolvesFileURL() {
        XCTAssertEqual(
            TerminalController.normalizeReportedDirectory("file:///Users/vmux/project"),
            "/Users/vmux/project"
        )
    }
...
final class TerminalControllerSocketListenerHealthTests: XCTestCase {
    ...
    func testNonStableSocketBindFailureDoesNotFallback() {
        XCTAssertNil(
            TerminalController.fallbackSocketPathAfterBindFailure(
                requestedPath: "/tmp/vmux-debug.sock",
                stage: "bind",
                errnoCode: EACCES,
                currentUserID: 501
            )
    ...
    private func makeTempSocketPath() -> String {
        "/tmp/vmux-socket-health-\(UUID().uuidString).sock"
    }
