...
final class CommandPaletteSwitcherSearchIndexerTests: XCTestCase {
    func testKeywordsIncludeDirectoryBranchAndPortMetadata() {
        let metadata = CommandPaletteSwitcherSearchMetadata(
            directories: ["/Users/example/dev/vmuxterm-hq/worktrees/feat-cmd-palette"],
        ...
        XCTAssertTrue(keywords.contains("/Users/example/dev/vmuxterm-hq/worktrees/feat-cmd-palette"))
    ...
    func testFuzzyMatcherMatchesDirectoryBranchAndPortMetadata() {
        let metadata = CommandPaletteSwitcherSearchMetadata(
            directories: ["/tmp/vmuxterm/worktrees/issue-123-switcher-search"],
    ...
    func testSurfaceDetailOutranksWorkspaceDetailForPathToken() {
        let metadata = CommandPaletteSwitcherSearchMetadata(
            directories: ["/tmp/worktrees/vmux"],
        ...
        let workspaceScore = try XCTUnwrap(
            CommandPaletteFuzzyMatcher.score(query: "vmux", candidates: workspaceKeywords)
        )
        let surfaceScore = try XCTUnwrap(
            CommandPaletteFuzzyMatcher.score(query: "vmux", candidates: surfaceKeywords)
        )
