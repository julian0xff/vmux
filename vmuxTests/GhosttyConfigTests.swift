...
final class GhosttyConfigTests: XCTestCase {
    ...
    func testThemeSearchPathsIncludeXDGDataDirsThemes() {
        let pathA = "/tmp/vmux-theme-a"
        let pathB = "/tmp/vmux-theme-b"
    ...
    func testLoadThemeResolvesPairedThemeValueByColorScheme() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("vmux-ghostty-theme-pair-\(UUID().uuidString)")
    ...
    func testLoadThemeResolvesBuiltinAliasFromGhosttyResourcesDir() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("vmux-ghostty-themes-\(UUID().uuidString)")
        let themesDir = root.appendingPathComponent("themes")
        try FileManager.default.createDirectory(at: themesDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testLoadThemeResolvesITerm2SolarizedLightAliasToLegacyThemeName() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("vmux-ghostty-solarized-light-\(UUID().uuidString)")
        let themesDir = root.appendingPathComponent("themes")
        try FileManager.default.createDirectory(at: themesDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testLoadThemeResolvesITerm2SolarizedDarkAliasToLegacyThemeName() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("vmux-ghostty-solarized-dark-\(UUID().uuidString)")
        let themesDir = root.appendingPathComponent("themes")
        try FileManager.default.createDirectory(at: themesDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testReleaseAppSupportFallbackLoadsForDebugWhenOnlyReleaseConfigExists() {
        XCTAssertTrue(
            GhosttyApp.shouldLoadReleaseAppSupportGhosttyConfig(
                currentBundleIdentifier: "com.vmuxterm.app.debug",
    ...
    func testReleaseAppSupportFallbackSkipsWhenDebugConfigAlreadyExists() {
        XCTAssertFalse(
            GhosttyApp.shouldLoadReleaseAppSupportGhosttyConfig(
                currentBundleIdentifier: "com.vmuxterm.app.debug.issue-829",
                currentConfigFileSize: nil,
    ...
    func testReleaseAppSupportFallbackSkipsForNonDebugBundleOrMissingReleaseConfig() {
        XCTAssertFalse(
            GhosttyApp.shouldLoadReleaseAppSupportGhosttyConfig(
                currentBundleIdentifier: "com.vmuxterm.app",
                currentConfigFileSize: nil,
                currentLegacyConfigFileSize: nil,
                releaseConfigFileSize: 128,
                releaseLegacyConfigFileSize: nil
    ...
    func testClaudeCodeIntegrationDefaultsToEnabledWhenUnset() {
        let suiteName = "vmux.tests.claude-hooks.\(UUID().uuidString)"
...
final class WindowTransparencyDecisionTests: XCTestCase {
    ...
    func testTranslucentOpacityForcesClearWindowBackgroundOutsideSidebarBlendModePath() {
        withTemporaryWindowBackgroundDefaults {
            ...
            XCTAssertFalse(vmuxShouldUseTransparentBackgroundWindow())
            XCTAssertTrue(vmuxShouldUseClearWindowBackground(for: 0.80))
            XCTAssertFalse(vmuxShouldUseClearWindowBackground(for: 1.0))
        }
    }
    ...
    func testBehindWindowGlassPathStillControlsTransparentWindowFallback() {
        withTemporaryWindowBackgroundDefaults {
            ...
            XCTAssertEqual(vmuxShouldUseTransparentBackgroundWindow(), expectedTransparentFallback)
            XCTAssertEqual(
                vmuxShouldUseClearWindowBackground(for: 1.0),
...
final class SocketControlSettingsTests: XCTestCase {
    func testMigrateModeSupportsExpandedSocketModes() {
        ...
        XCTAssertEqual(SocketControlSettings.migrateMode("vmuxOnly"), .vmuxOnly)
    ...
    func testSocketModePermissions() {
        ...
        XCTAssertEqual(SocketControlMode.vmuxOnly.socketFilePermissions, 0o600)
    ...
    func testInvalidEnvSocketModeDoesNotOverrideUserMode() {
        XCTAssertNil(
            SocketControlSettings.envOverrideMode(
                environment: ["VMUX_SOCKET_MODE": "definitely-not-a-mode"]
            )
        )
        XCTAssertEqual(
    ...
    func testStableReleaseIgnoresAmbientSocketOverrideByDefault() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "VMUX_SOCKET_PATH": "/tmp/vmux-debug-issue-153-tmux-compat.sock",
            ],
    ...
    func testNightlyReleaseUsesDedicatedDefaultAndIgnoresAmbientSocketOverride() {
        let path = SocketControlSettings.socketPath(
            ...
            bundleIdentifier: "com.vmuxterm.app.nightly",
            isDebugBuild: false,
            probeStableDefaultPathEntry: { _ in .missing }
        )

        XCTAssertEqual(path, "/tmp/vmux-nightly.sock")
    }

    func testDebugBundleHonorsSocketOverrideWithoutOptInFlag() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "VMUX_SOCKET_PATH": "/tmp/vmux-debug-my-tag.sock",
            ],
            bundleIdentifier: "com.vmuxterm.app.debug.my-tag",
            isDebugBuild: false
        )

        XCTAssertEqual(path, "/tmp/vmux-debug-my-tag.sock")
    }

    func testStagingBundleHonorsSocketOverrideWithoutOptInFlag() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "VMUX_SOCKET_PATH": "/tmp/vmux-staging-my-tag.sock",
            ],
            bundleIdentifier: "com.vmuxterm.app.staging.my-tag",
            isDebugBuild: false
        )

        XCTAssertEqual(path, "/tmp/vmux-staging-my-tag.sock")
    }

    func testStableReleaseCanOptInToSocketOverride() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "VMUX_SOCKET_PATH": "/tmp/vmux-debug-forced.sock",
                "VMUX_ALLOW_SOCKET_OVERRIDE": "1",
            ],
            bundleIdentifier: "com.vmuxterm.app",
            isDebugBuild: false,
            probeStableDefaultPathEntry: { _ in .missing }
        ...
        XCTAssertEqual(path, "/tmp/vmux-debug-forced.sock")
    }
    ...
    func testDefaultSocketPathByChannel() {
        ...
        XCTAssertEqual(
            ...
            "/tmp/vmux-nightly.sock"
        )
        XCTAssertEqual(
            SocketControlSettings.defaultSocketPath(
                bundleIdentifier: "com.vmuxterm.app.debug.tag",
                isDebugBuild: false,
                probeStableDefaultPathEntry: { _ in .missing }
            ),
            "/tmp/vmux-debug.sock"
        ...
        XCTAssertEqual(
            SocketControlSettings.defaultSocketPath(
                bundleIdentifier: "com.vmuxterm.app.staging.tag",
                isDebugBuild: false,
                probeStableDefaultPathEntry: { _ in .missing }
            ),
            "/tmp/vmux-staging.sock"
        )
    }
    ...
    func testUntaggedDebugBundleBlockedWithoutLaunchTag() {
        XCTAssertTrue(
            SocketControlSettings.shouldBlockUntaggedDebugLaunch(
                environment: [:],
                bundleIdentifier: "com.vmuxterm.app.debug",
                isDebugBuild: true
            )
        )
    }
    ...
    func testTaggedDebugBundleAllowedWithoutLaunchTag() {
        XCTAssertFalse(
            SocketControlSettings.shouldBlockUntaggedDebugLaunch(
                environment: [:],
                bundleIdentifier: "com.vmuxterm.app.debug.tests-v1",
                isDebugBuild: true
            )
        )
    }
    ...
    func testXCUITestLaunchEnvironmentIgnoresLaunchTagGate() {
        ...
        // The app receives VMUX_UI_TEST_* vars via XCUIApplication.launchEnvironment.
        XCTAssertFalse(
            SocketControlSettings.shouldBlockUntaggedDebugLaunch(
                environment: ["VMUX_UI_TEST_MODE": "1"],
                bundleIdentifier: "com.vmuxterm.app.debug",
                isDebugBuild: true
            )
        )
...
final class GhosttyMouseFocusTests: XCTestCase {
    ...
    private func withTempConfig(
        ...
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("vmux-test-cjk-\(UUID().uuidString)")
    ...
    func testUserConfigContainsCJKCodepointMapReturnsFalseForMissingFiles() {
        let path = NSTemporaryDirectory() + "vmux-nonexistent-\(UUID().uuidString)/config"
        XCTAssertFalse(
    ...
    func testUserConfigContainsCJKCodepointMapFollowsConfigFileIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("vmux-test-cjk-include-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigContainsCJKCodepointMapFollowsRelativeIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("vmux-test-cjk-rel-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigContainsCJKCodepointMapHandlesOptionalInclude() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("vmux-test-cjk-opt-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigContainsCJKCodepointMapHandlesCyclicIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("vmux-test-cjk-cycle-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
...
final class ZshShellIntegrationHandoffTests: XCTestCase {
    func testGhosttyPromptHooksLoadWhenVmuxRequestsZshIntegration() throws {
        let output = try runInteractiveZsh(vmuxLoadGhosttyIntegration: true)
    ...
    func testGhosttyPromptHooksDoNotLoadWithoutVmuxHandoffFlag() throws {
        let output = try runInteractiveZsh(vmuxLoadGhosttyIntegration: false)

        XCTAssertTrue(output.contains("PRECMD=0"), output)
        XCTAssertTrue(output.contains("PREEXEC=0"), output)
    }

    private func runInteractiveZsh(vmuxLoadGhosttyIntegration: Bool) throws -> String {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("vmux-zsh-shell-integration-\(UUID().uuidString)")
        ...
        let repoRoot = URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent()
        let vmuxZdotdir = repoRoot.appendingPathComponent("Resources/shell-integration")
        ...
        process.environment = [
            ...
            "SHELL": "/bin/zsh",
            "USER": NSUserName(),
            "ZDOTDIR": vmuxZdotdir.path,
            "VMUX_ZSH_ZDOTDIR": userZdotdir.path,
            "VMUX_SHELL_INTEGRATION": "0",
            "GHOSTTY_RESOURCES_DIR": ghosttyResources.path,
        ]
        if vmuxLoadGhosttyIntegration {
            process.environment?["VMUX_LOAD_GHOSTTY_ZSH_INTEGRATION"] = "1"
        }
