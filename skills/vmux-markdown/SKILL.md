...
# Absolute path
vmux markdown open /path/to/PLAN.md

# Target a specific workspace
vmux markdown open design.md --workspace workspace:2
```
...
When creating a plan or task list, write it to a `.md` file and open it in vmux:
...
```bash
# Open in the caller's workspace (default -- uses VMUX_WORKSPACE_ID)
vmux markdown open plan.md

# Open in a specific workspace
vmux markdown open plan.md --workspace workspace:2

# Open splitting from a specific surface
vmux markdown open plan.md --surface surface:5

# Open in a specific window
vmux markdown open plan.md --window window:1
```
