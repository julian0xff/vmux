...
vmux uses isolated browser contexts per surface. Treat each browser surface as its own session.
...
# session B
vmux browser open https://example.com --json
# -> surface:8

vmux browser surface:7 get url
vmux browser surface:8 get url
```
...
```bash
vmux browser surface:7 state save /tmp/auth-state.json
```

### Load State

```bash
vmux browser surface:8 state load /tmp/auth-state.json
...
vmux browser open https://app.example.com/login --json
# login on surface:7 ...
vmux browser surface:7 state save /tmp/auth.json

vmux browser open https://app.example.com --json
# assume surface:8
vmux browser surface:8 state load /tmp/auth.json
vmux browser surface:8 goto https://app.example.com/dashboard
```
...
```bash
vmux browser open https://site-a.example --json
vmux browser open https://site-b.example --json
vmux browser open https://site-c.example --json

vmux browser surface:11 get text body > /tmp/a.txt
vmux browser surface:12 get text body > /tmp/b.txt
vmux browser surface:13 get text body > /tmp/c.txt
```
...
vmux close-surface --surface surface:8
