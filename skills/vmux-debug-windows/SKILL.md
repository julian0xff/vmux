---
name: vmux-debug-windows
description: Manage vmux debug windows and related debug menu wiring for Sidebar Debug, Background Debug, and Menu Bar Extra Debug. Use this when the user asks to open/tune these debug controls, add or adjust Debug menu entries, or capture/copy a combined debug config snapshot.
---
...
1. Verify debug menu wiring in `Sources/vmuxApp.swift` under `CommandMenu("Debug")`.
...
```bash
skills/vmux-debug-windows/scripts/debug_windows_snapshot.sh --copy
```
5. After code edits, run build + tagged reload:
```bash
xcodebuild -project GhosttyTabs.xcodeproj -scheme vmux -configuration Debug -destination 'platform=macOS' build
./scripts/reload.sh --tag <tag>
```

## Key Files

- `Sources/vmuxApp.swift`: Debug menu entries and debug window controllers/views.
...
Examples:
```bash
skills/vmux-debug-windows/scripts/debug_windows_snapshot.sh
skills/vmux-debug-windows/scripts/debug_windows_snapshot.sh --copy
skills/vmux-debug-windows/scripts/debug_windows_snapshot.sh --domain <bundle-id> --copy
```
