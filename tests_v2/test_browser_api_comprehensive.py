...
def _wait_until(pred, timeout_s: float, label: str) -> None:
    ...
    if last_exc is not None:
        raise vmuxError(f"Timed out waiting for {label}: {last_exc}")
    raise vmuxError(f"Timed out waiting for {label}")
...
def _expect_error(label: str, fn, code_substr: str) -> None:
    try:
        ...
        if code_substr in text:
            return
        raise vmuxError(f"{label}: expected error containing {code_substr!r}, got: {text}")
    raise vmuxError(f"{label}: expected error containing {code_substr!r}, but call succeeded")
...
def _expect_error_contains(label: str, fn, *needles: str) -> None:
    try:
        ...
        if missing:
            raise vmuxError(f"{label}: missing expected substrings {missing!r} in error: {text}")
        return
    raise vmuxError(f"{label}: expected failure, but call succeeded")
...
def _wait_with_fallback(c: vmux, surface_id: str, params: dict, pred, timeout_s: float, label: str) -> None:
...
<html>
  <head>
    <title>vmux-browser-comprehensive-1</title>
    <style>
...
<!doctype html>
<html>
  <head><title>vmux-browser-comprehensive-2</title></head>
  <body>
...
def main() -> int:
    ...
    with vmux(SOCKET_PATH) as c:
        ...
        _wait_until(
            lambda: "vmux-browser-comprehensive-1"
        ...
        c._call("browser.fill", {"surface_id": target, "selector": "#name", "text": "vmux"})
        ...
        _must(str(_value(out_text)) == "vmux", f"Expected status text to be vmux: {out_text}")
        ...
        _must(str(_value(name_val)) == "vmux-v2", f"Expected typed suffix in input value: {name_val}")
        ...
        _must("vmux-browser-comprehensive-1" in snapshot_text, f"Expected snapshot text for page1: {snap}")
        ...
        _wait_until(
            lambda: "vmux-browser-comprehensive-2"
            in str((c._call("browser.get.title", {"surface_id": target}) or {}).get("title") or ""),
            timeout_s=3.0,
        ...
        _wait_with_fallback(
            c,
            target,
            {"url_contains": "vmux-browser-comprehensive-1", "timeout_ms": 4000},
            lambda: "vmux-browser-comprehensive-1" in str((c._call("browser.url.get", {"surface_id": target}) or {}).get("url") or ""),
            timeout_s=5.0,
        ...
        _wait_with_fallback(
            c,
            target,
            {"url_contains": "vmux-browser-comprehensive-2", "timeout_ms": 4000},
            lambda: "vmux-browser-comprehensive-2" in str((c._call("browser.url.get", {"surface_id": target}) or {}).get("url") or ""),
            timeout_s=5.0,
