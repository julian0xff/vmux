...
def _rename_surface(client: vmux, surface_id: str, title: str) -> None:
    client._call(
        "surface.action",
        {
...
def _current_surface_id(client: vmux, workspace_id: str) -> str:
...
def main() -> int:
    with vmux(SOCKET_PATH) as client:
        ...
        if len(rows) < 2:
            raise vmuxError(f"expected at least two surfaces after split: {payload}")
        ...
        if not left_surface_id:
            raise vmuxError(f"failed to resolve left surface id: {payload}")
        ...
        if not result_rows:
            raise vmuxError("switcher returned no rows for renamed surface query")
