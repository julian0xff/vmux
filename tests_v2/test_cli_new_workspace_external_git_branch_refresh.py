...
def _resolve_socket_path() -> str:
    socket_path = os.environ.get("VMUX_SOCKET", "").strip()
    if not socket_path:
        raise vmuxError("VMUX_SOCKET is required (expected /tmp/vmux-debug-<tag>.sock)")
    if not re.fullmatch(r"/tmp/vmux-debug-[^/]+\.sock", socket_path):
        raise vmuxError(f"VMUX_SOCKET must be a tagged debug socket, got: {socket_path!r}")
    return socket_path
...
def _find_cli_binary() -> str:
    ...
    candidates = glob.glob(
        os.path.expanduser("~/Library/Developer/Xcode/DerivedData/**/Build/Products/Debug/vmux"),
        recursive=True,
    )
...
def _create_git_repo(root: Path) -> Path:
    ...
    subprocess.run(
        ["git", "config", "user.name", "vmux-test"],
        cwd=repo,
        check=True,
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
    ...
    subprocess.run(
        ["git", "config", "user.email", "vmux-test@example.com"],
        cwd=repo,
        check=True,
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
...
def main() -> int:
    ...
    temp_root = Path(tempfile.mkdtemp(prefix="vmux_issue_915_external_git_"))
