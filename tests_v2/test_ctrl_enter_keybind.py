...
def infer_app_name_for_osascript(socket_path: str) -> str:
    ...
    Examples:
      - /tmp/vmux-debug.sock          -> "vmux DEV"
      - /tmp/vmux-debug-foo.sock      -> "vmux DEV foo"
      - ~/Library/Application Support/vmux/vmux.sock -> "vmux"
      - /tmp/vmux-foo.sock            -> "vmux foo"
    """
    base = Path(socket_path).name
    if base.startswith("vmux-debug") and base.endswith(".sock"):
        suffix = base[len("vmux-debug") : -len(".sock")]
        if suffix.startswith("-") and suffix[1:]:
            return f"vmux DEV {suffix[1:]}"
        return "vmux DEV"
    if base.startswith("vmux") and base.endswith(".sock"):
        suffix = base[len("vmux") : -len(".sock")]
        if suffix.startswith("-") and suffix[1:]:
            return f"vmux {suffix[1:]}"
        return "vmux"
...
def test_ctrl_enter_keybind(client: vmux) -> tuple[bool, str]:
...
def run_tests() -> int:
    print("=" * 60)
    print("vmux Ctrl+Enter Keybind Test")
    print("=" * 60)
    print()
