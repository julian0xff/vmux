...
def _expect_not_supported(c: vmux, method: str, params: dict) -> None:
    try:
        ...
        if "not_supported" in text:
            return
        raise vmuxError(f"Expected not_supported for {method}, got: {text}")
    raise vmuxError(f"Expected not_supported for {method}, but call succeeded")
