...
def wait_for_notifications(client: vmux, expected: int, timeout: float = 2.0) -> list[dict]:
    start = time.time()
...
def wait_for_flash_count(client: vmux, surface: str, minimum: int = 1, timeout: float = 2.0) -> int:
...
def ensure_two_surfaces(client: vmux) -> list[tuple[int, str, bool]]:
...
def focused_surface_index(client: vmux) -> int:
    surfaces = client.list_surfaces()
...
def send_osc(client: vmux, sequence: str, surface: Optional[int] = None) -> None:
...
def test_clear_prior_notifications(client: vmux) -> TestResult:
...
def test_suppress_when_focused(client: vmux) -> TestResult:
...
def test_not_suppressed_when_inactive(client: vmux) -> TestResult:
...
def test_kitty_notification_simple(client: vmux) -> TestResult:
...
def test_kitty_notification_chunked(client: vmux) -> TestResult:
...
def test_rxvt_notification_osc777(client: vmux) -> TestResult:
...
def test_mark_read_on_focus_change(client: vmux) -> TestResult:
...
def test_mark_read_on_app_active(client: vmux) -> TestResult:
...
def test_mark_read_on_tab_switch(client: vmux) -> TestResult:
...
def test_flash_on_tab_switch(client: vmux) -> TestResult:
...
def test_focus_on_notification_click(client: vmux) -> TestResult:
...
def test_restore_focus_on_tab_switch(client: vmux) -> TestResult:
...
def test_clear_on_tab_close(client: vmux) -> TestResult:
