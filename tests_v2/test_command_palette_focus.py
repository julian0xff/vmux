...
def _focused_surface_id(client: vmux) -> str:
...
def main() -> int:
    token = "VMUX_PALETTE_FOCUS_PROBE_9412"
    restore_token = "VMUX_PALETTE_RESTORE_PROBE_7731"
    ...
    with vmux(SOCKET_PATH) as client:
        ...
        if token in post_text and token not in pre_text:
            raise vmuxError("typed probe text leaked into terminal while palette is open")
        ...
        if restore_token not in restore_text:
            raise vmuxError("terminal did not receive typing after closing command palette")
