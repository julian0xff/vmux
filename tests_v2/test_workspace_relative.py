#!/usr/bin/env python3
"""Regression: CLI commands are workspace-relative via VMUX_WORKSPACE_ID.

Tests that when VMUX_WORKSPACE_ID is set, CLI commands target that workspace
...
def _find_cli_binary() -> str:
    env_cli = os.environ.get("VMUXTERM_CLI")
    if env_cli and os.path.isfile(env_cli) and os.access(env_cli, os.X_OK):
        return env_cli

    fixed = os.path.expanduser("~/Library/Developer/Xcode/DerivedData/vmux-tests-v2/Build/Products/Debug/vmux")
    if os.path.isfile(fixed) and os.access(fixed, os.X_OK):
        return fixed

    candidates = glob.glob(os.path.expanduser("~/Library/Developer/Xcode/DerivedData/**/Build/Products/Debug/vmux"), recursive=True)
    candidates += glob.glob("/tmp/vmux-*/Build/Products/Debug/vmux")
    candidates = [p for p in candidates if os.path.isfile(p) and os.access(p, os.X_OK)]
    if not candidates:
        raise vmuxError("Could not locate vmux CLI binary; set VMUXTERM_CLI")
...
def _run_cli(cli: str, args: List[str], env_overrides: Optional[Dict[str, str]] = None) -> str:
    ...
    if proc.returncode != 0:
        ...
        raise vmuxError(f"CLI failed ({' '.join(cmd)}): {merged}")
...
def _run_cli_json(cli: str, args: List[str], env_overrides: Optional[Dict[str, str]] = None) -> Any:
    ...
    try:
        ...
        raise vmuxError(f"Invalid JSON output: {output!r} ({exc})")


def test_list_panels_workspace_relative(c: vmux, cli: str) -> None:
    ...
    payload_env = _run_cli_json(
        cli, ["list-panels"],
        env_overrides={"VMUX_WORKSPACE_ID": ws_a["id"]}
    )
...
def test_list_panes_workspace_relative(c: vmux, cli: str) -> None:
...
def test_send_workspace_relative(c: vmux, cli: str) -> None:
    """send with VMUX_WORKSPACE_ID env var targets that workspace's surface."""
    ws_result = c._call("workspace.list")
    workspaces = ws_result.get("workspaces", [])
    _must(len(workspaces) >= 1, "Need at least 1 workspace")
    ...
    output = _run_cli(
        cli, ["send", " "],
        env_overrides={"VMUX_WORKSPACE_ID": ws["id"]}
    )
...
def test_send_with_explicit_workspace(c: vmux, cli: str) -> None:
...
def test_v2_migrated_commands_output_refs(c: vmux, cli: str) -> None:
...
def test_surface_health_workspace_relative(c: vmux, cli: str) -> None:
...
def test_non_json_output_uses_refs(c: vmux, cli: str) -> None:
