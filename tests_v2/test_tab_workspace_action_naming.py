...
def _run_cli_json(cli: str, args: List[str]) -> Dict:
    ...
    try:
        ...
        raise vmuxError(f"Invalid JSON output for {' '.join(args)}: {output!r} ({exc})")


def _focused_surface_ref(c: vmux, workspace_id: str) -> str:
    ...
    raise vmuxError(f"Unable to resolve focused surface ref in workspace {workspace_id}: {listed}")
