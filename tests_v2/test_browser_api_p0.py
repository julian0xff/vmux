...
<!doctype html>
<html>
  <head><title>vmux-browser-p0</title></head>
  <body>
    <input id='name' value=''>
...
""".strip()
        ...
        _must("vmux" in str(out.get("value", "")), f"Expected #out text to include 'vmux': {out}")
        ...
        _must(str(eval_res.get("value", "")) == "vmux", f"Expected eval value 'vmux': {eval_res}")

        snap = c._call("browser.snapshot", {"surface_id": target}) or {}
        snapshot_text = str(snap.get("snapshot") or "")
        _must("vmux-browser-p0" in snapshot_text, f"Expected snapshot text to include page title: {snap}")
    ...
    print("PASS: browser.* P0 methods work on vmux webview with ref handles")
    return 0
