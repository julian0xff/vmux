...
def _surface_row(c: vmux, workspace_id: str, surface_id: str) -> dict:
...
def _devtools_visible(c: vmux, workspace_id: str, surface_id: str) -> bool:
    row = _surface_row(c, workspace_id, surface_id)
    return bool(row.get("developer_tools_visible"))


def _focus_browser_webview(c: vmux, surface_id: str, timeout_s: float = 2.0) -> None:
    deadline = time.time() + timeout_s
    last_exc = None
    while time.time() < deadline:
        try:
            ...
            last_exc = exc
        time.sleep(0.05)
    raise vmuxError(f"Timed out waiting for browser webview focus: {last_exc}")
