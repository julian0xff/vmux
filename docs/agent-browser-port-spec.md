...
This document tracks implemented behavior and remaining parity gaps for the vmux browser port.

## Goals

1. Provide an LLM-friendly browser automation API in vmux with stable handles.
...
1. `./scripts/run-tests-v1.sh` passes on `vmux-vm`.
2. `./scripts/run-tests-v2.sh` passes on `vmux-vm`.
...
6. `browser.tab.*` compatibility aliases mapped to vmux surfaces
7. `browser.console.list`
8. `browser.errors.list`
9. `browser.highlight`
10. `browser.state.save|load` (browser state in vmux context)
...
Primary form:
```bash
vmux browser --surface <surface-id> <agent-browser-style-command...>
```

Shorthand:
```bash
vmux browser <surface-id> <agent-browser-style-command...>
```
...
vmux identify
vmux capabilities
vmux browser identify --surface <surface-id>   # wrapper over system.identify + browser fields
```

Flash:
```bash
vmux trigger-flash [--workspace <id>] [--surface <id>]
```
...
- [x] Add `browser` command group in `CLI/vmux.swift` that accepts agent-browser-style command grammar.
...
3. `src/protocol.test.ts` -> adapted browser command/shape validation in `tests_v2/test_browser_api_unsupported_matrix.py` and existing `CLI/vmux.swift` command grammar checks.
...
5. `src/daemon.test.ts`, `src/stream-server.test.ts`, `test/serverless.test.ts`, `src/ios-manager.test.ts` -> out-of-scope for vmux browser parity (different transport/runtime).

### Implemented vmux Browser Suites
...
1. `ssh vmux-vm 'cd /Users/vmux/GhosttyTabs && ./scripts/run-tests-v2.sh'`
2. `ssh vmux-vm 'cd /Users/vmux/GhosttyTabs && ./scripts/run-tests-v1.sh'`

## Decision Log (Locked - February 12, 2026)

1. `vmux browser tab ...` maps to browser `surface` tabs only (no separate workspace-level tab meaning inside `browser` namespace).
...
5. Output shape is vmux-native overall, but `browser.snapshot` and selector `not_found` diagnostics intentionally mirror agent-browser semantics for agent usability.
...
2. Whether to expose protocol-only agent-browser actions in first public release of `vmux browser` or gate them behind a second rollout phase.
