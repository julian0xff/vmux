...
## Goal
Ensure commands run through the vmux Unix socket/CLI do not steal user focus from the current UI workflow.
...
- [x] Add explicit `rename-tab` command (defaults to `VMUX_TAB_ID` / `VMUX_SURFACE_ID` / `VMUX_WORKSPACE_ID` when flags omitted).
