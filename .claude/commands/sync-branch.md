...
1. **Update submodules to latest**
   - For each submodule (ghostty, homebrew-vmux, vendor/bonsplit):
     - `cd <submodule>`
     - `git fetch origin`
...
2. **Commit submodule updates on main**
   ...
   - If changed, stage and commit: `git add ghostty homebrew-vmux vendor/bonsplit && git commit -m "Update submodules: <brief description>"`
