...
If the script fails, run `say "vmux release failed"`.

### 7. Verify homebrew cask

- Run `bash tests/test_homebrew_sha.sh` to confirm the cask SHA matches the release DMG
- Update the homebrew-vmux submodule pointer: `git add homebrew-vmux && git commit -m "Update homebrew-vmux submodule to latest" && git push origin main`
