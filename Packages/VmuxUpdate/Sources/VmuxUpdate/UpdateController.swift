...
public class UpdateController {
    ...
    public func startUpdaterIfNeeded() {
        ...
        if ProcessInfo.processInfo.environment["VMUX_UI_TEST_RESET_SPARKLE_PERMISSION"] == "1" {
            let defaults = UserDefaults.standard
...
#endif
        do {
            // vmux never enables automatic update checks; we rely on the in-app update pill.
    ...
    private func recordUITestTimestamp(key: String) {
        ...
        guard let path = env["VMUX_UI_TEST_TIMING_PATH"] else { return }
